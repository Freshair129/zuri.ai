...
| 7 `DPS-KI-CHUNK` | parsed text/heading sections → persisted exact-substring chunks | hash/UTF-16 offsets/ordinal ตรง parent. **TEXT profile (2026-09-24 remediation):** `genesisrag17-parser-3` / `genesisrag17-chunker-2` bounds every window by whichever of the 80-whitespace-token budget or a new 480-character budget is hit first (480 = 512 e5 `MAX_LENGTH` − 2 special tokens − 9 worst-case tokens for the 9-character `"passage: "` prefix, margin 21 — see Stage 15); cuts at paragraph → sentence-punctuation/Thai-run-space → any-whitespace → hard cut (last resort), keeps ≤60-character overlap between windows of the same section, never opens a chunk on a combining mark or inside a surrogate pair. `genesisrag17-parser-1` / `genesisrag17-chunker-1` are historical identities only, kept for rows already parsed under them — no new ingestion produces them, and the gap this row recorded in 1.4.2b is now closed for new ingestions only; a persisted intent recorded under the historical identity still resumes/replays unchanged through the old splitter (2026-09-25 follow-up, `isHistoricalParserIdentity`), and the same follow-up fixed the overlap step, which could stall near a boundary and emit near-duplicate/whitespace-only slivers, and made the overlap start actually boundary-searched. A later 2026-09-25 follow-up (`c924e5d1` plus the blank-line fix, gate rounds 4–5) found that sliver fix incomplete for ordinary prose: a window starting at an overlap start re-found the paragraph break that made the previous cut and emitted an overlap-plus-one-character sliver, and a budget edge inside a run of blank lines emitted a chunk adding only whitespace. The cut search now starts past the previous cut and past any whitespace there, and a chunk that adds no non-whitespace text beyond the previous chunk is not emitted (`tests/unit/genesisrag17-chunker-safety-fixes.test.js`). This changes chunk boundaries for ordinary prose too (a short paragraph followed by one longer than a window), so the 1.4.8b statement that ordinary prose output was unchanged no longer holds. Avoiding the input-value 400 was not enough by itself: Stage 2 (`ensureParsedArtifact`) separately 409'd on the same resume/replay because the legacy metadata carried `maxChars: null, overlapChars: null` instead of omitting both keys, changing its content hash relative to every already-persisted parser-1 row — a second 2026-09-25 follow-up fixed this by omitting them, matching the pre-remediation metadata shape exactly (`docs/plans/GENESISRAG17-CONTRACT.md` 1.4.4b; proven by a real ingest -> replay -> Stage 2 integration test). Every existing TEXT-profile provider gets new chunk ids/boundaries for a new admission, including `LINE_FAQ_CANDIDATE` (FR-236) and `LINE_STUDIO_DESCRIPTION` (FR-238). **A THIRD 2026-09-25 follow-up (1.4.8b)** closed the "480 characters is always safe" claim's own gap: the pinned tokenizer's NFKC normalization can expand some Unicode compatibility characters well past one token per character (measured: 480 x U+FDFA -> 8,640 normalized characters), so every window is now also bounded by `text.normalize('NFKC').length <= 480`; it also fixed the overlap search still landing on the exact boundary that produced the cut (zero overlap on most sentence/whitespace cuts) by requiring the overlap boundary strictly earlier, and capped the forward grapheme-safety nudge to the character budget instead of letting it grow unbounded when no safe boundary existed at all (measured pre-fix: single 1,001/1,500-character chunks) — see `docs/plans/GENESISRAG17-CONTRACT.md` 1.4.5b. parser-2: หนึ่ง section = หนึ่ง chunk ไม่ตัดตาม token (unchanged) | Z-source parser/chunk helper, Z-executor `ensureChunks` |
...
| 15 `DPS-KI-EMBED` | allowed chunks → real CPU E5 vectors, model/hash evidence | 384 dimensions cosine, exact pinned artifacts; policy denial/model error ไม่ทำ fake vectors. Tokenizer window จริง `MAX_LENGTH=512` รวม prefix `passage:`/`query:`; `enable_truncation` ตัดปลายแบบเงียบ ไม่มี reject path. **2026-09-24 remediation:** a Stage 7 chunk from `genesisrag17-parser-3` is now bounded at 480 characters, which stays under 512 tokenizer tokens for ordinary prose in any script in the worst case of one token per character (`480 = 512 − 2 special − 9 worst-case "passage: " prefix tokens`, margin 21) — so a new-ingestion chunk can no longer silently truncate here for realistic input (measured: the DoD's 3,000-character Thai paragraph tops out at ~142 tokenizer tokens per 480-character chunk). **2026-09-25 follow-up (1.4.8b):** that arithmetic is not a universal guarantee on its own — the pinned tokenizer's NFKC normalization can expand some Unicode compatibility characters well past one token per character (measured: 480 x U+FDFA -> 8,640 normalized characters) — so Stage 7 now also bounds every window by `text.normalize('NFKC').length <= 480`, closing the gap for that pathological case too. A `genesisrag17-parser-1` chunk (historical rows only) is unaffected and can still exceed the window per the note this row previously recorded | W-worker + Python embedder |
...
| Z-structured | [genesisrag17-structured-record.js](../apps/server/src/modules/knowledge/genesisrag17-structured-record.js) (parser-2 rendering + structured recognizer, FR-188); [parser-2 unit tests](../apps/server/tests/unit/genesisrag17-parser-2.test.js), [parser-2 Tier 1 tests](../apps/server/tests/integration/genesisrag17-parser-2.test.js) |
...
|---|---|---|---|---|---|
| 1.4.9b | 2026-09-25 | beta | Corrects the sliver claims of 1.4.6b/1.4.8b: two ordinary-prose sliver paths remained (a window re-finding the previous paragraph-break cut; a budget edge inside a run of blank lines). The cut search now starts past the previous cut and past whitespace, and a chunk adding no non-whitespace text is not emitted. Chunk boundaries change for ordinary prose too, superseding 1.4.8b's 'ordinary prose output is unchanged'. Proven by `tests/unit/genesisrag17-chunker-safety-fixes.test.js`; fuzz of 24,000 generated documents: 0 slivers, 0 mid-word cuts, 0 chunks over 480 raw/NFKC characters; parser-1 historical and parser-2 unchanged | working-tree | Claude Opus 5.5 |
| 1.4.8b | 2026-09-25 | beta | Closes three gaps an Opus gate review found in the 1.4.5b Thai-safe-chunker remediation (arithmetic in `docs/plans/GENESISRAG17-CONTRACT.md` 1.4.5b): the 480-character budget now also bounds `text.normalize('NFKC').length` (the pinned tokenizer's NFKC step can expand some Unicode compatibility characters past one token per character); the overlap boundary search now requires a result strictly earlier than the cut (it could previously re-find the exact boundary that produced the cut, delivering zero overlap on most sentence/whitespace cuts); the forward grapheme-safety nudge is now capped to the character budget instead of growing unbounded when no safe boundary exists at all. Proven by `tests/unit/genesisrag17-chunker-safety-fixes.test.js`; parser-2 unaffected | working-tree | Claude Sonnet 5 |
| 1.4.7b | 2026-09-25 | beta | Correction to 1.4.6b: the historical resume/replay claim was only true of the input-value 400 — Stage 2's `ensureParsedArtifact` still 409'd on the same resume/replay because the legacy path nulled rather than omitted `metadata.maxChars`/`overlapChars`, and that field feeds the content-identity hash. Fixed by omitting both keys for a legacy request, restoring metadata byte-identical to the pre-remediation shape; a `splitRange` overlap corner case on malformed input also closed. Proven by a real ingest -> FR-071 replay -> Stage 2 integration test and a pinned-hash regression unit test | working-tree | Claude Sonnet 5 |
| 1.4.6b | 2026-09-25 | beta | Fixes to 1.4.5b: overlap step could stall near a boundary and emit near-duplicate/whitespace-only slivers instead of making forward progress (fixed); overlap start now actually boundary-searched, not only grapheme-nudged; a persisted `genesisrag17-parser-1`/`-chunker-1` intent now resumes/replays through the historical splitter unchanged instead of 400/409ing; names both live TEXT-profile LINE OA providers affected (`LINE_FAQ_CANDIDATE` FR-236, `LINE_STUDIO_DESCRIPTION` FR-238) | working-tree | Claude Sonnet 5 |
| 1.4.5b | 2026-09-24 | beta | Stage 7 row records the new TEXT-profile `genesisrag17-parser-3` / `genesisrag17-chunker-2` character/boundary/overlap windowing (closes the 1.4.1b/1.4.2b gap for new ingestions); Stage 15 row cross-references the arithmetic keeping a chunk under the 512-token embedder window; `genesisrag17-parser-1`/`-chunker-1` recorded as historical-only; no stage ownership, wire field or SMARTGIFT_CATALOG parser-2 behavior changed | working-tree | Claude Sonnet 5 |
| 1.4.4b | 2026-09-24 | beta | Gate-review fixes: Stage 5 row now says FR-238 (`LINE_STUDIO_DESCRIPTION`) is explicitly excluded from the new gate, corrects the evidence claim (persisted Stage 5 evidence carries only `errorCode`, never the policy identity or rule name — those are on the thrown error only), and corrects the `ZERO_PII_POLICY_BY_PROVIDER` line reference from `:90` to `:104` | working-tree | Claude Sonnet 5 |

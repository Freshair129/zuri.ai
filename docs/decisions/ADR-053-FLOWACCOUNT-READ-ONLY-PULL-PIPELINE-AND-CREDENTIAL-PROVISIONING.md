...
> ## ⚠ Provider facts in this document were verified on **2026-08-20**. Re-verify before implementing.
>
> Every FlowAccount base URL, scope, token lifetime, page-size ceiling, rate
> limit and endpoint listed here was enumerated against FlowAccount's live
> OpenAPI specification and published documentation on **2026-08-20**, and has
> **not** been re-checked since. Rate limits and pagination ceilings in
> particular have already moved twice in 2026 (2026-05-29 and 2026-07-09 — see
> §2), so treat every number below as a **dated observation, not a current
> fact**. Phase 0 of §10 must re-enumerate the specification and reconcile it
> against this document before any code is written.

**Relates to:** [ADR-032](ADR-032-INTEGRATION-SECRET-MANAGEMENT-UI.md), [ADR-038](ADR-038-MARKET-INTELLIGENCE-DOMAIN-BOUNDARY.md), [FR-080](../domains/integration/features/FR-080-integration-secret-management-ui.md), [FR-081](../domains/integration/features/FR-081-raw-external-ingestion.md), [FR-125](../domains/integration/features/FR-125-flowaccount-read-only-pull-pipeline.md)

> **Id note.** This decision was drafted on 2026-08-20 as `ADR-039` against
> `FR-094`, and additionally declared `NFR-019`, `BR-020`, `SDD-052` and
> `SEC-018`. Every one of those numbers was claimed on main by unrelated work
> before this branch was re-applied — `ADR-039` is the requirement-id subject
> anchor decision, `FR-094` is the canonical IAM principal, and the four
> supporting ids belong to the IAM and knowledge-ingestion slices. Ids are keys
> (AGENTS.md §18): the earlier declarations keep their meaning and this document
> carries **ADR-053 / FR-125**, the same rule PR #117 followed for the sibling
> control-roadmap rescue and PR #180 for the plugin-auth rescue.
>
> The four supporting declarations were **dropped rather than renumbered**. Their
> substance survives here and in FR-125's acceptance criteria; re-declaring them
> would burn a number in four separate registry families for a document that
> authorizes no implementation. The implementation PR that acts on this ADR
> should declare them fresh, against whatever numbers are free at that time.
...
| Provider facts verified | **2026-08-20** — re-verify before implementing |
...
Phase 1 model-provider metadata (`purpose=PHASE1_LINE_LLM`) และรับเพียง opaque
reference รูปแบบ `supabase-vault:<uuid>` ตาม ADR-032/FR-080 — หน้าเดียวกันนี้
*แสดง* LINE_OA `CHANNEL` connection ที่ระบบอื่นสร้างไว้ได้ แต่ยังไม่สามารถ
onboard FlowAccount, ทดสอบ credential, ดึงข้อมูล, แสดง run หรือบอกจุดที่ sync
ล้มเหลวได้
...
> **ยืนยันเมื่อ 2026-08-20 — ยังไม่ได้ตรวจซ้ำ ต้อง re-verify ก่อน implement.**
> ตรวจจากเอกสารทางการและ OpenAPI specification ที่เผยแพร่ ณ วันนั้น ทุกค่าใน
> หัวข้อนี้เป็น *ข้อสังเกตที่มีวันที่กำกับ* ไม่ใช่ข้อเท็จจริงปัจจุบัน
...
2. Client Credentials Flow เป็นแบบ 1 Client ต่อ 1 บริษัท FlowAccount และใช้
   ...
   ในหน้า Client Credentials ณ 2026-08-20 พิมพ์ `grant_type=refresh_token` ซึ่ง
   ขัดกับเนื้อหาในหน้าเดียวกัน จึงถือเป็น provider-doc discrepancy และต้องยืนยัน
   กับ Sandbox contract test ก่อน implementation
...
6. ตั้งแต่ 2026-07-09 rate limit คือ Sandbox 20 requests/minute และ Production
   ...
   exponential backoff
7. การ enumerate OpenAPI specification ณ 2026-08-20 พบ 130 paths และไม่พบ
   webhook หรือ callback contract ดังนั้น V1 ต้องเป็น server-initiated pull
   ไม่อ้างว่า FlowAccount push event เข้ามา
...
**ข้อ 5, 6 และ 7 คือค่าที่เปลี่ยนบ่อยที่สุด** — เพดาน pagination ประกาศเปลี่ยน
เมื่อ 2026-05-29 และ rate limit เมื่อ 2026-07-09 ภายในปีเดียวกัน การรับค่าเหล่านี้
มาใช้โดยไม่ตรวจซ้ำคือความเสี่ยงที่เอกสารนี้เตือนไว้โดยตรง

แหล่งอ้างอิงทางการ (URL ตามที่เข้าถึงเมื่อ 2026-08-20):
...
configuration drift ส่วน custom base URL ถูกห้ามเพื่อปิด SSRF — ค่าคงที่ทั้งสอง
อ้างจากข้อเท็จจริงข้อ 2 ซึ่งยืนยันเมื่อ 2026-08-20 และมี provider-doc
discrepancy ค้างอยู่ จึงต้องผ่าน Sandbox contract test ก่อนตรึงเป็นค่าคงที่จริง
...
ค่าที่เอกสารนี้ **เสนอ** (ยังไม่มีอยู่ใน repository วันนี้ — ดู "สถานะจริงของ
repository" ท้าย D4):
...
identifiers เท่านั้น ไม่เป็น primary key ของ Zuri (BR-002) หาก FlowAccount record
ไม่มี stable external id adapter ต้องส่งเข้า dead letter ด้วย
`FLOWACCOUNT_EXTERNAL_ID_MISSING` แทนการสร้าง id จาก page/row position

หนึ่ง Business ควรมีได้หนึ่ง ACTIVE FlowAccount connection ต่อ
...
**สถานะจริงของ repository (ตรวจ 2026-08-30):** ค่าทั้งห้าบรรทัดข้างบนเป็น
*ข้อเสนอ* ไม่ใช่ vocabulary ที่มีอยู่

- `CONNECTION_KINDS` ใน `src/platform/integrations/core/connection-health.js`
  คือ `['CHANNEL', 'MODEL_PROVIDER']` เท่านั้น — **`DATA_SOURCE` ยังไม่มี** และ
  การเพิ่มเป็น kind ที่สามคือ schema/enum change ที่ต้องได้รับอนุมัติแยก
- `IntegrationConnection.authorizationType` มี default `"SECRET_MANAGER"` และ
  `purpose` มี default `"GENERAL"`; `OAUTH2_CLIENT_CREDENTIALS` และ
  `ACCOUNTING_DATA_PULL` เป็นค่าใหม่ที่เอกสารนี้เสนอ
- unique constraint ที่มีอยู่จริงคือ
  `@@unique([tenantId, providerId, externalAccountId])` — เป็นระดับ **Tenant**
  และ **ไม่มี** `purpose` อยู่ในคีย์ ข้อจำกัด "หนึ่ง ACTIVE connection ต่อ
  provider + externalAccountId + purpose" จึงต้องบังคับด้วย constraint ใหม่หรือ
  invariant ระดับ service ไม่ใช่สิ่งที่ schema วันนี้รับประกันให้แล้ว
...
adapter ต้องเรียก `ingestRawExternalRecord` เดิม
(`src/platform/integrations/core/raw-ingest-service.js`) และประกอบ envelope ด้วย
`zIngestionEnvelope` เดิม (`src/platform/integrations/core/contracts.js`)
ไม่สร้าง persistence path ที่สอง ทุก envelope ระบุ trusted
Tenant/Business/connection จาก server scope, `provider=FLOWACCOUNT`,
`sourceType=PULL`, lane, entity type, external id, schema version, source URI ที่
ไม่มี credential, payload และ payload hash

### D6 — Resource catalog V1 เป็น allow-list แบบ read-only

> **Endpoint paths ยืนยันเมื่อ 2026-08-20 — re-verify ก่อน implement.**
> ตารางนี้เป็น provider fact ที่มีวันที่กำกับ path หรือ resource ที่ FlowAccount
> เพิ่ม/ย้าย/ถอนหลังจากวันนั้นจะไม่ปรากฏที่นี่

| Resource code | FlowAccount GET (as of 2026-08-20) | Lane | External ID | Pull strategy |
|---|---|---|---|---|
...
Lane ทุกค่าในตารางนี้เป็นสมาชิกของ `DATA_LANES` ที่มีอยู่จริงใน
`src/platform/integrations/core/contracts.js` แล้ว จึงไม่ต้องเพิ่ม lane ใหม่
...
ทั่วไป (ตามที่ enumerate เมื่อ 2026-08-20) ดังนั้น:
...
> **เพดาน 20/100 requests/minute เป็นค่าที่ยืนยันเมื่อ 2026-08-20** (ประกาศ
> 2026-07-09) — re-verify ก่อน implement ตัวเลข headroom ข้างล่างต้องคำนวณใหม่
> จากเพดานจริง ณ วันที่เริ่ม implement ไม่ใช่จากตัวเลขในเอกสารนี้

- account-wide limiter ตั้งเป้าสูงสุด 15 req/min Sandbox และ 80 req/min
  Production เพื่อเหลือ headroom จากเพดาน 20/100 ที่บันทึกไว้
...
ซึ่ง `connection-health.js` ประกาศไว้แล้ว แต่ `DATA_SOURCE` เป็น kind ใหม่ที่
เอกสารนี้เสนอ (ดู D4) และไม่ใช้ inbound-traffic silence แบบ `CHANNEL` — วันนี้
`connection-health.js` ใช้ `trafficMatters` เฉพาะเมื่อ `kind === 'CHANNEL'`
เท่านั้น จึงเป็น extension point ที่รองรับ kind ใหม่ได้โดยไม่แก้ semantics เดิม:
...
ทุกแถวในตารางนี้เป็น route ที่ **เสนอ** — วันนี้ repository มีเพียง
`GET`/`POST /api/platform/integrations` และ
`/api/platform/integrations/line-registry`
...
re-resolve viewer และ `ownsBusiness` (`src/modules/identity/viewer-authority.js`)
ก่อนอ่าน connection หรือเรียก external API ทุกครั้ง id ของ connection ภายนอก
scope ตอบเหมือนไม่มีอยู่
...
|---|---|---:|---|
| 0 | **re-verify provider facts §2 กับ OpenAPI ปัจจุบัน** แล้วยืนยัน scope/resources, Sandbox access, retention และ V1 auth mode | 1–2d | owner sign-off + usable sandbox credential + §2 ที่ตรวจใหม่พร้อมวันที่ |
...
|---|---|---|---|
| **provider facts ในเอกสารนี้เก่าไปแล้ว** | High | High | §2 มีวันที่กำกับทุกจุด; Phase 0 บังคับ re-enumerate ก่อนเขียน code |
...
4. revert application deployment/schema additive changes ตาม migration plan
...
6. `DATA_SOURCE` เป็น connection kind ที่สาม (ดู D4) — เจ้าของยืนยันการเพิ่ม kind
   นี้เข้า `CONNECTION_KINDS` และ enum/constraint ที่ตามมาหรือไม่
...
ก่อนเริ่ม code ของแต่ละ phase ต้องเสนอ documentation diff และได้รับ approval จาก
เจ้าของอีกครั้ง ตามลำดับงาน governance ใน CLAUDE.md (declare id → chartered lane →
annotate → `npm run govern`) การอนุมัติเอกสารนี้ **ไม่** อนุญาตให้แก้ code, schema,
secret manager หรือเรียก FlowAccount ด้วย credential จริง
...
| 0.2.0b | 2026-08-30 | candidate | Re-applied to main from `rescue/flowaccount-docs` as ADR-053/FR-125 (ids renumbered; NFR/BR/SDD/SEC declarations dropped, see Id note). Provider facts marked as verified 2026-08-20 and unre-checked. Corrected four claims about this repository that no longer held 239 commits later: `DATA_SOURCE` is not an existing connection kind, `OAUTH2_CLIENT_CREDENTIALS`/`ACCOUNTING_DATA_PULL` are not existing enum values, the connection uniqueness constraint is tenant-scoped and excludes `purpose`, and the "R5" approval rule no longer exists | pending | Claude Opus 5 |

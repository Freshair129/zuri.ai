...
4. **Durable admission, acknowledged on durable capture, reconciled from evidence.** `POST /api/line-oa/accounts/{id}/webhook` verifies signature before parsing, including empty verification requests. Account ID is a locator, not authorization. Destination must match the server-owned connection. Every event — text or not — is first recorded as sanitized Integration evidence, and **that** write is what HTTP 200 acknowledges. This sentence said "before HTTP 200" until 2026-09-10 and it was wrong twice over: admission runs ~25–30 sequential queries against a remote Postgres and took 7–11 s in production, so on 2026-09-09 LINE gave up on every genuinely new message and redelivered the same `webhookEventId` four times at 62 s intervals; the boundary moved to the write that makes the event unloseable (PR #306). Admission then runs asynchronously in the same long-lived container — the ADR-058 Node container makes this safe where a serverless runtime would drop it — retried on transient failure, and still creates account-scoped CRM inbound and a uniquely keyed job in one transaction. Asynchronous is not fire-and-forget: the evidence row is labelled `ADMITTING` before the first attempt, and the server worker's bounded tick re-admits rows stranded there past a threshold that clears the in-process retry ladder, from the payload already stored. A reconciled event has no reply token — evidence strips it at capture and LINE's token is long dead by then — so its job is created without one and leaves by Push where the account permits delayed Push, or ends `FAILED` where it does not; it is never quietly discarded, and a terminal failure is counted on the Studio conversation surface rather than left for a database query. Non-text events are acknowledged without invented support. Group text requires an explicit bot mention before creating a response job. Transient LINE reply tokens never enter raw evidence or logs.
...
6. **Worker and delivery recovery.** A server worker claims durable work using compare-and-set versions and bounded leases, and one tick may hold more than one claim at a time. Until 2026-09-10 it held exactly one: a tick claimed a single job, ran the model call inside itself and returned, so two customers who wrote in the same moment were answered strictly in series — the second waiting out the first's whole answer, up to the supervisor's 240 s request timeout. That was never required by this decision; the compare-and-set fencing named in this very sentence exists so that concurrent claimants are safe. A tick now answers up to a bounded number of jobs in parallel and then sends up to a bounded number of ready answers in order (amended 2026-09-10). Sends stay sequential deliberately: they are sub-second and LINE rate-limits per account, so parallelism there would buy nothing and earn 429s. Both bounds are deployment-configurable, because the cost of a wide tick is metered model calls, not durability. The supervisor's polling cadence follows the work rather than a fixed interval — quick while there is any, backing off to a ceiling while there is none — and admission wakes the worker endpoint directly when it queues a job, so the idle ceiling is not what a waiting customer experiences. That wake-up is an optimisation with no authority of its own: it calls the same deployment-authenticated bounded endpoint the supervisor calls, and the supervisor's own loop remains the guarantee that queued work is picked up. SERVER work calls the established scoped business-answer contract; for a job whose admission captured the immutable memory opt-in, its answer adapter may compose the trusted MSP thread port and later reconcile the same persisted CRM receipt through a bounded CAS scanner. Jobs without that opt-in retain the public-knowledge/no-memory path, and the adapter never calls LINE or creates a second CRM row. EDGE work waits for an authenticated device. The final answer is persisted before sending. Prefer Reply API for a still-valid sealed reply token; after a send with an ambiguous outcome never switch to Push, because that can double-send. A Reply the provider *definitively* rejected is the one case that does switch, and it is an application of that rule rather than an exception to it: `LINE_HTTP_400` from the Reply endpoint is a confirmed non-delivery of a dead reply token, so falling back to Push cannot double-send anything (amended 2026-09-10). When a Reply fails that way and the account permits delayed Push, the job returns to READY with `sendMethod` switched to PUSH and the already-persisted answer is pushed without recomputing it. Every other Reply failure is unchanged: ambiguous outcomes — 5xx, timeout, a lost response — stay UNKNOWN and never switch, and the credential/configuration rejections that share the same `PERMANENT_FAILURE` transport mapping (401, 403, 404) stay terminally FAILED, because Push would fail on them the same way under a different name. Delayed work may use Push only when the account explicitly permits it. Push has an immutable recipient/body/retry UUID from its first attempt, exponential backoff and a retry deadline below LINE's 24-hour key lifetime. A 409 is acceptance only with the provider's accepted-request identifier. Acceptance is not delivery/read.

...
4. **Durable admission.** `POST /api/line-oa/accounts/{id}/webhook` verifies signature before parsing, including empty verification requests. Account ID is a locator, not authorization. Destination must match the server-owned connection. Text events create account-scoped CRM inbound and a uniquely keyed job in one transaction before HTTP 200. Non-text events are preserved as sanitized Integration evidence and acknowledged without invented support. Group text requires an explicit bot mention before creating a response job. Transient LINE reply tokens never enter raw evidence or logs.

...
| `supabase/migrations/20260829120000_profile_identity_fields.sql` | the production lineage — applied by hand, its filename maps 1:1 with the `schema_migrations` ledger row |
| `prisma/postgres/0001_init.sql` | regenerated by `npm run db:pg:sql`; **not** hand-extended with an `0003_` |
...
| `tests/unit/profile-identity-fields-migration.test.js` | the production DDL is additive with no `NOT NULL`, and — the guard that was missing — `prisma/postgres/` holds **only** the regenerated `0001` and the one hand-written `0002`, with `0001` proven in step with the schema |

## Where the production DDL belongs, and why it moved

This shipped once with the DDL in `prisma/postgres/0003_profile_identity_fields.sql`,
which is the wrong lane, and no test caught it.

`prisma/postgres/0001_init.sql` is **regenerated whole** by `npm run db:pg:sql`
(`prisma migrate diff --from-empty`). A hand-added `0003_` is therefore not an
increment on it — it is a second, diverging source of truth that a replay would
apply on top of an init already containing the same change. Production DDL
belongs in `supabase/migrations/`, whose filenames map 1:1 with the rows in
`supabase_migrations.schema_migrations`; `0002` is the one legitimate
hand-written extra, for a partial unique index Prisma cannot express.

Nothing failed because every migration test pins the one file it knows about by
name. A *missing* file has no test to fail. The directory-shape assertion added
here is the guard that can see it.

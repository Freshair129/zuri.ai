...
> **Current transport authority: ADR-061 (2026-09-06), amended by [ADR-100](../../decisions/ADR-100-LINE-OA-RUNS-SERVER-EXECUTED-ON-BROWSER-PROVISIONED-API-KEYS.md) (2026-09-21).**
> The owner approved server-owned LINE ingress/send; the optional EDGE compute half
> is **retired** (FR-265). `transportMode` admits CLOUD alone, execution is always
> SERVER, and this lane no longer owns `src/app/api/edge/conversation-jobs/**` —
> those routes are withdrawn. Device pairing and ADR-059 asset extraction are
> untouched and were never this lane's. Every server answer calls a real model under
> the Business's own API key, provisioned in the browser through the Integration
> lane (FR-266); the `modelAccess` LOCAL_ONLY branch, which substituted a canned
> deterministic answerer rather than calling a local model, is gone with it.
> Existing transport descriptions below describe ADR-060 legacy/planned lanes and
> are superseded for conversational transport by ADR-061 and ADR-100.

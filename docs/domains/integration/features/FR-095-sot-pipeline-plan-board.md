...
# FR-095 — SoT pipeline plan board
...
FR-095 gives the plan one authoritative, viewable home: a board of the pipeline's
...
2. **Status is derived, not stored.** For each phase, status =
   ...
   - `blocked` — the newest linked run FAILED, or a required FR-096 decision for
   ...
   route composes it from `listPipelineRuns` + FR-096 decision counts.
3. **Viewer-scoped.** `GET /api/platform/sot/plan` resolves the viewer
   ...
   per-phase run/decision counts. No mutation endpoints exist in FR-095.
4. **One board page.** `/platform/sot-pipeline` renders the phases in dependency
   ...
   navigation into FR-096's inbox and FR-071's run evidence.

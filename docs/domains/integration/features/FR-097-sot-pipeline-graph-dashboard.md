...
# FR-097 — SoT pipeline graph dashboard
...
shows is already available from FR-095 (plan + derived status) and FR-096
...
1. **Same data shape as the dependency map.** The dashboard consumes
   ...
   (`sot-pipeline-graph.js`) from the FR-095 plan + status payload: one node per
...
3. **Status legend is FR-095's.** `planned / running / blocked / done` plus a
   pending-decision badge (count) on phases that wait on a human — the badge
   links into the FR-096 inbox filtered to that phase.

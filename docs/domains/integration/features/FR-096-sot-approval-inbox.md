...
# FR-096 — SoT approval inbox and decision export
...
FR-096 closes it with one queue: the pipeline **submits** pending decisions, a
...
1. **One model.** `SotDecision`: tenant/business-scoped;
   ...
   an external id, never a primary key, BR-002); `phaseId` (links to FR-095);
...
queue keeps its own contract; FR-096 is the generic data-fact queue).

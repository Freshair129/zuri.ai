...
### Reproducible snapshot (owner-approved, 2026-09-12)

Schema `2.0` removes the generation-time `generatedAt` field. Identical source
inputs must produce identical serialized state even when generation runs at a
different time. Both `docs/.domain-state.json` and the app's
`runtime/domain-state.json` contain the same bytes. Run time belongs in the
ignored graph diagnostic report or CI logs, not the committed snapshot.

The dashboard states that the snapshot is bundled with this app version and is
not live telemetry; it does not invent a replacement date. Evidence paths,
readiness calculations and authorization retain their existing meaning.
Consumers of schema `1.1` must update with the generator and its bundled output;
no database or external API migration is involved.

Verification changes the clock between two builds and compares the complete
serialized result. CI checks committed generated state before regeneration and
checks the regenerated files against Git before tests/build, preserving the
stale-output gate while giving state-consuming tests a coherent projection.
...
- The methodology, bundled-snapshot provenance and stale-snapshot boundary are visible.

...
| FEAT-014 | CRM Conversation Intelligence — the derived-intelligence layer over the FR-023 LINE ingress: an AI-inferred per-Customer profile, per-conversation analysis records, and a per-Business Daily Sales Brief pushed over LINE; table shapes borrowed from the legacy ERD as prior art and rebound to this product's scope chain (ADR-054). Use case: the operator of a Business opens the day knowing which of yesterday's LINE conversations are HOT, what customers kept asking for, and who is worth calling back — without rereading a single thread | FR-126, FR-127, FR-128 | building |

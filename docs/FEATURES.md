...
| FEAT-013 | Knowledge Ingestion Governance — the governance and isolated execution layer over the seventeen-stage knowledge ingestion pipeline (ADR-071; extension points in docs/KNOWLEDGE-INGESTION-17-STAGE-FLOW.md): the stage catalog and end-to-end job trace carried on the FR-071 execution ledger, the published-snapshot contract that lets an answer name the corpus it read, and the sensitivity/processing-policy lattice that decides what may be indexed and where each stage may run | FR-109, FR-110, FR-111 | building |
...
[
  ...
  },
  {
    ...
    "useCase": "เจ้าของธุรกิจเชื่อมบัญชี LINE Official Account หลายบัญชีเข้ากับ Business เดียว เห็นสถานะการเชื่อมต่อ binding และ webhook ล่าสุดของแต่ละบัญชีในที่เดียว และกำหนดว่าบัญชีไหนส่งผ่าน Zuri Edge Device ของตนเองหรือผ่านคลาวด์ โดยไม่เห็นความลับของ LINE"
  },
  {
    "id": "FR-001",
    "primaryDomain": "project-manager",

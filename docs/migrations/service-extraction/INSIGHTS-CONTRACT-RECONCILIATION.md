...
- **D-09 Notification adapter, credential and recipients stay separate (R-16, B5):** `createLinePushReportNotifier` takes an
  ...
  deterministic hash of `{syncRunId, failureKey, recipientKind, recipientId}` (RFC 4122 UUID v5) computed once per
  recipient inside the push loop, never shared across recipients — LINE checks `X-Line-Retry-Key` per request, so one
  key reused across recipients would get every push after the first rejected as a duplicate of it, leaving only the
  first recipient ever alerted (a real bug caught in review of the first version of this adapter and fixed before any
  transport binding existed). A repeated call for the same failure to the same recipient still relies on the

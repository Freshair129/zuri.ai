...
| F-18 | Two concurrent commits of ONE cost sheet on PostgreSQL READ COMMITTED: the loser read the sheet as DRAFT before the winner committed, went on to the Product carton compare-and-swap, waited on the winner's row and then matched 0 rows, answering 409 `PRODUCT_VERSION_CONFLICT` instead of the contract's replay. SQLite's writer lock always produced the replay, so the answer depended on timing. Legacy `supplier-cost-sheet-service` has the same unlocked shape | Reproduced in SCM: 5/120 runs under 12-way parallel load (same sheet `["committed","PRODUCT_VERSION_CONFLICT","replayed","replayed"]`); a forced overlap (a third connection holds the Product row until both commits wait) gives it every time | **FIXED in SCM `bc24e5bb`** (D-28): a DRAFT sheet is locked (lock-only touch) and re-read before deciding; forced-overlap regression case (PostgreSQL-only) fails before the fix and passes after; 240 loop runs under the same load, 0 failures. No assertion loosened | Legacy: **reproduced on the legacy code** 3/3 with a deterministic barrier: the loser crashed with an unmapped Prisma unique-constraint error on `supplierCostLine.create` (nothing written twice). **Fix in [#572](https://github.com/Freshair129/zuri.ai/pull/572)** (draft, head `e6e27e79`): the sheet is re-read under the supplier lock before any status decision; race file 15/15 x3; npm test 6982 pass / 47 skip; build clean; govern exit 0. S1 **PASS** at `e6e27e79`; hosted CI green on attempt 3 (attempts 1–2 hit an unrelated Knowledge flake, F-20); marked ready; **MERGED** into `main` at `a91aa1e7` (2026-09-25T10:53:08Z) under the owner's merge authority. Not deployed by S5 |
| F-20 | Not SCM's: `knowledge-runtime.test.js:786` failed 2 of 3 CI runs on #572's merge ref ("closed 2 instead of 1") while main passed. Root cause in the Knowledge lane: the orphan sweep (`knowledge-runtime.js` ~:275–305) re-queries pages by status + lease only, and a row it closed gets no new lease. So when page 1 is full (19–20 older refused rows due in the same pass), page 2 returns the just-closed row, and the sweep counts it closed twice | Reproduced deterministically in a read-only investigation (19 due rows gives the exact CI result; 18 passes). The count of due rows depends on runner speed (real-time leases from the previous test). The knowledge file runs first on a fresh DB in every run, so #572 is not involved | Routed to MC0 and, on the user's instruction (2026-09-25), to the gr17 / KI17 lane, which confirmed the root cause and opened **draft [#573](https://github.com/Freshair129/zuri.ai/pull/573)** (head `43690ecd`): the sweep tracks the ids it has already processed in one pass, plus a clock-independent regression test. S1 PASS at `7bf8d1f1` after three test-only rounds; **MERGED** into `main` at `d302eb08` by MC0. **CLOSED** | Knowledge owner: fix the double count (track closed ids, or refresh between pages) and make :786 deterministic |

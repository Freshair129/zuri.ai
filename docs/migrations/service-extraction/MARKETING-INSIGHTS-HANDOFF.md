...
reaching an alert; a deterministic UUIDv5 retry key from `{syncRunId, failureKey}`; and the LINE notifier
returning the honest unavailable port on a missing credential reference or an empty/invalid recipient list,
never guessing either one.

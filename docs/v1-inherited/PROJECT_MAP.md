...
> Source: `G:/zuri/docs/PROJECT_MAP.md` @ `0b6d3c3` · imported 2026-08-11
...
> **Version:** 2.0.0 · **Date:** 2026-08-05 (was 1.2.0, 2026-04-09)
> Single source of truth สำหรับ navigate codebase ทั้งหมด
> Counts: 84 pages · 196 route handlers · 85 components · 51 repositories · 89 Prisma models
...
│   ├── app/                      ← Next.js 14 App Router — 84 pages, 196 route handlers
│   │   ├── (auth)/               ← Public routes (login, register)
│   │   ├── (dashboard)/          ← Protected routes (21 top-level modules)
│   │   └── api/                  ← API endpoints (127 route.js) + api/workers/ (24)
│   ├── components/               ← React components (per domain) — 85
│   ├── modules/                  ← Business logic (ADR-060)
│   ├── lib/                      ← db.ts (tenant scoping) + repositories/ (51) + ai/, billing/,
│   │                                pdf/, line/, facebook/, accounting/, kpi/, settings/
...
│
├── prisma/                       ← DB schema (single source of truth) — 89 models
├── gks/                          ← Genesis Knowledge System (KB SSOT)
│   ├── 00_index/                 ← L0 atomic index — scan this FIRST
│   ├── phase1_docs/              ← Product narrative
│   ├── phase2_atomic/            ← ADR / MOD / FEAT / PROTO / FLOW (live ADRs → ADR-085)
│   ├── phase3_blueprints/        ← YAML blueprints (28 FEATs)
│   ├── phase3.5_microtasks/      ← Micro-task YAMLs for local codegen
│   ├── 06_incidents/             ← Production post-mortems
│   └── 14_devlog/                ← task/ · implement/ · reviews/ · experiment/
├── .brain/msp/                   ← MSP gatekeeper — LLM_Contract/ + projects/G--zuri/inbound/
├── mobile/                       ← Expo Router native app (POS)
├── automation/                   ← Codex gated feature runner (FEAT-027)
├── scripts/msp/                  ← index builder, validators, codegen runner + composer
├── docs/                         ← Obsidian vault (human-facing documentation)
├── .claude/                      ← Claude Code config + skills
├── .zdev/                        ← Legacy dev-tool registry (superseded by gks/)
...
├── CRITICAL_FACTS.md             ← Live risk flags — read first
└── CLAUDE.md                     ← Root agent context (AGENTS.md points here)
```
...
├── layouts/    DashboardShell · Sidebar · Topbar · BottomNav
...
| บันทึกงานรายวัน (Devlog) | [[devlog/index]] |

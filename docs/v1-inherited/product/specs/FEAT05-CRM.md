...
> Source: `G:/zuri/docs/product/specs/FEAT05-CRM.md` @ `0b6d3c3` · imported 2026-08-11
...
**Depends On:** FEAT01 (tenantId, Customer model)
**Related:** FEAT02 (Customer profile), FEAT04 (Inbox identity), FEAT14 (AI insights)
...
| **Customer ID** | unique ID ของ customer ในรูปแบบ `{CODENAME}-CUS{CH}{YYMM}-{NNNN}` (e.g. `TVS-CUSFB2603-0001`) ตาม `id_standards.yaml` |
...
- ระบบ DSB ใช้ stage history นี้คำนวณ conversion rate รายวัน (ดู FEAT10-DSB.md)
...
FB Messenger (PSID: xxx) ──┐
                            ├──▶ Customer Record เดียว (TVS-CUS{CH}{YYMM}-{NNNN})
LINE OA (userId: yyy) ──────┘
...
### 3.5b Workflow Automation — User Stories
- **OWN-07:** ในฐานะเจ้าของ ฉันต้องการให้ระบบส่ง LINE ตามหา lead อัตโนมัติเมื่อไม่ได้ติดต่อนาน 3 วัน โดยไม่ต้องให้ staff จำ
- **MGR-06:** ในฐานะผู้จัดการ ฉันต้องการรับ notification ทันทีเมื่อ AI ตรวจจับว่าลูกค้าพร้อมซื้อ
- **OWN-08:** ในฐานะเจ้าของ ฉันต้องการให้ระบบส่ง reminder ชำระเงินอัตโนมัติเมื่อ student ลงทะเบียนแล้วยังไม่จ่ายภายใน 48 ชม.
- **MGR-07:** ในฐานะผู้จัดการ ฉันต้องการดูสถิติว่า workflow ไหน trigger ไปกี่ครั้ง มี conversion กี่ %
...
|---|---|---|
| `CONVERSATION` | บทสนทนา FB/LINE — คลิกไปเปิด Inbox ได้ | FEAT04-INBOX.md |
| `ORDER` | ออเดอร์จาก POS — แสดงรายการสินค้า + ยอดรวม | FEAT06-POS.md |
| `ENROLLMENT` | การลงเรียน course — แสดงชื่อคอร์ส + วันที่ | Enrollment module |
| `PAYMENT` | ประวัติการชำระเงิน — แสดง slip / invoice | FEAT03-BILLING.md |
...
- Columns: Customer ID, name, phone, email, stage, tags, assigned_agent, created_at, last_contact, total_spend
...
ระบบทำงานอัตโนมัติเมื่อ customer เข้า condition ที่กำหนด — ไม่ต้องให้ staff follow-up เอง
...
| Trigger | รายละเอียด |
|---|---|
| `STAGE_CHANGED` | lifecycle stage เปลี่ยนเป็น stage ที่กำหนด |
| `TIME_SINCE_LAST_CONTACT` | ไม่มีการติดต่อนานเกิน N วัน |
| `ENROLLMENT_NO_PAYMENT` | ลงทะเบียนแล้วยังไม่ชำระเงินนาน N ชม. |
| `PURCHASE_INTENT` | AI วิเคราะห์ว่า intent = `READY_TO_BUY` |
| `TAG_ADDED` | เพิ่ม tag บางอันให้ customer |
| `PIPELINE_STAGE_CHANGED` | เปลี่ยน conversation pipeline stage เป็น stage ที่กำหนด |

#### Actions

| Action | รายละเอียด |
|---|---|
| `SEND_LINE_MESSAGE` | ส่ง LINE message template ให้ customer (ต้องมี LINE userId) |
| `SEND_FB_MESSAGE` | ส่ง FB message template ให้ customer (ต้องมี PSID) |
| `NOTIFY_AGENT` | แจ้งเตือน agent ที่ assigned ผ่าน Pusher + in-app notification |
| `CHANGE_STAGE` | เปลี่ยน lifecycle stage อัตโนมัติ |
| `ASSIGN_AGENT` | assign agent ตาม round-robin หรือ rule |
| `ADD_TAG` | เพิ่ม tag ให้ customer |

#### ตัวอย่าง Workflow ที่ใช้บ่อย (culinary school)

| ชื่อ | Trigger | Condition | Action |
|---|---|---|---|
| Follow-up lead หาย | TIME_SINCE_LAST_CONTACT | stage IN (INTERESTED) AND > 3 วัน | SEND_LINE_MESSAGE (template: "สวัสดีครับ ยังสนใจคอร์ส...") |
| แจ้ง agent lead ร้อน | PURCHASE_INTENT | intent = READY_TO_BUY | NOTIFY_AGENT |
| Reminder ชำระเงิน | ENROLLMENT_NO_PAYMENT | > 48 ชม. | SEND_LINE_MESSAGE (template: "ยืนยันการลงทะเบียน...") |
| Welcome enrolled | STAGE_CHANGED | stage = ENROLLED | SEND_LINE_MESSAGE (template: "ยินดีต้อนรับสู่คอร์ส...") |
| Re-engage lost lead | TIME_SINCE_LAST_CONTACT | stage = CONTACTED AND > 14 วัน | SEND_LINE_MESSAGE + ADD_TAG("re-engage") |

#### Architecture

```
QStash cron ทุก 1 ชม. → /api/workers/automation-check
  → ดึง active workflows ของทุก tenant
  → evaluate triggers + conditions per customer
  → enqueue actions → /api/workers/automation-execute
  → execute action (send message / notify / update stage)
  → log ผลลัพธ์ใน automation_logs
```

- Workflow เปิด/ปิดได้ต่อ tenant
- 1 customer trigger workflow เดิมได้ไม่เกิน 1 ครั้งต่อ 24 ชม. (cooldown ป้องกัน spam)
- ทุก action บันทึกลง `customer_activities` timeline
...
┌─────────────────────────────────────────┐
│ Follow-up Lead หาย          [เปิดใช้ ✓] │
│                                          │
│ Trigger: ไม่มีการติดต่อ > 3 วัน         │
│ Condition: stage = INTERESTED            │
│ Action: ส่ง LINE — "ยังสนใจคอร์ส..."   │
│                                          │
│ ใช้งานแล้ว 47 ครั้ง  [แก้ไข] [ลบ]      │
└─────────────────────────────────────────┘
```
...
| CRM-P3-01 | Workflow Automation — Trigger + Condition + Action engine | P2 |
| CRM-P3-02 | Workflow Builder UI — สร้าง/แก้ไข workflow ใน Settings | P2 |
| CRM-P3-03 | QStash automation-check worker — evaluate ทุก 1 ชม. | P2 |
| CRM-P3-04 | automation_logs table + timeline entries | P2 |
| CRM-P3-05 | Cooldown guard (1 trigger ต่อ customer ต่อ 24 ชม.) | P2 |
...
**Feature Specs:**
- FEAT02-PROFILE.md — Customer Profile panel ใน Inbox sidebar (identity resolution detail, ads attribution)
- FEAT04-INBOX.md — Unified Inbox: conversations link กลับมา customer record ใน CRM
- FEAT10-DSB.md — Daily Sales Brief ใช้ lifecycle stage + conversion data จาก CRM
- FEAT03-BILLING.md — Payment history per customer แสดงใน Activity Timeline
- FEAT06-POS.md — Order history per customer แสดงใน Activity Timeline
- FEAT11-AI-ASSISTANT.md — NL2SQL query อาจ query `customers` table โดยตรง

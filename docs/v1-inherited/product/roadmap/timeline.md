...
> Source: `G:/zuri/docs/product/roadmap/timeline.md` @ `0b6d3c3` · imported 2026-08-16
...
date: "2026-04-11"
status: APPROVED
owner: ["Boss (Product)", "Antigravity (Lead Architect)"]
approach: "DOC TO CODE — ไม่ implement จนกว่า spec + ADR approved"
---

# Zuri Platform — Product Roadmap v2.3.0
...
│   ├── M1: Foundation + DOC (Apr)  ← ✅ DONE (All ADRs approved)
│   ├── M2: Core Migration (Apr)    ← ✅ DONE (UAT scripts integrated)
│   └── M3: AI Chat Intelligence — Wave 1 (Jun)
│
├── Q3 (Jul-Sep)
│   ├── M4: AI Agent + Sales Tools — Wave 2 (Jul-Aug) ← ✅ ACHIEVED (Sales Closer, Ads Audit)
│   └── M5: Multi-Tenant Launch (Sep) ← ✅ ACHIEVED (Onboarding Flow live)
│
└── Q4 (Oct-Dec)
    ├── M6: AI Ads + Content — Wave 3 (Oct-Nov) ← 🔄 ACTIVE (Session Pinning complete)
...
> **Status (2026-04-09): Implementation phases 4–6 COMPLETE + UI pages built (27 pages, 35 components). Phase 7 (UAT) pending.**
...
| | | | |
| **UI** | Landing Page (Hero + Pricing + CTA) | — | ✅ Done |
| **UI** | Overview Dashboard (KPI cards + trend) | — | ✅ Done |
| **UI** | Quick Actions page | — | ✅ Done |
| **UI** | POS Mobile page (3 payment, 3 order types) | — | ✅ Done |
| **UI** | POS Table Management (zone + table CRUD) | — | ✅ Done |
| **UI** | POS Seating Monitor (real-time 30s) | — | ✅ Done |
| **UI** | Theme Toggle (Dark/Light) + Amber Citrus design | — | ✅ Done |
| **UI** | Language Switcher (TH/EN) | — | ✅ Done |

**Deliverables:**
- [x] ~57 models migrated + working (54 Prisma models)
- [x] V School feature parity กับ ZURI v3.7
- [x] Unit tests pass (24 files, 55 tests)
- [x] 27 UI pages built (25 functional + 2 stubs)
- [x] 35 components (inbox 8, kitchen 3, marketing 4, pos 2, schedule 2, shared 5, ui 6, layouts 3, crm 2)
- [x] 68+ API routes
- [x] i18n (TH/EN) + Dark/Light theme
- [ ] E2E tests pass
...
**M4 Deliverables:**
- [x] A2: Agent Mode per-admin style
- [x] A3: AI Sales Closer (with human gate)
- [x] B2: Sales KPI Dashboard
- [x] C1: Ads Audit Report
- [x] **E1: Accounting Platform** — FlowAccount + Express X-import
...
**M5 Deliverables:**
- [x] Tenant onboarding flow (signup → provision → seeding)
...
#### D3: Session Pinning & Concurrent Control

```
Implemented Security module enforcing max 2 sessions, session valid per unique device footprint.
```

| Item | Spec |
|---|---|
| Module | core/auth, shared/security |
| Storage | ActiveSession model |
| Integration | NextAuth callbacks, Edge Middleware |
| Security | Pin by IP & User-Agent, Max 2 concurrent sessions |

**M6 Deliverables:**
- [x] D3: Session Pinning & Concurrent Control

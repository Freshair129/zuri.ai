...
> Source: `G:/zuri/docs/guide/DEV_SETUP.md` @ `0b6d3c3` · imported 2026-08-11
...
LINE_CHANNEL_SECRET
ACCOUNTING_ENCRYPTION_KEY   # 64-char hex (`openssl rand -hex 32`) — src/lib/crypto.js
TOKEN_ENCRYPTION_KEY        # 64-char hex (`openssl rand -hex 32`) — src/lib/crypto/tokenEncryption.js
RESEND_API_KEY              # FEAT21 invite + OTP email (lazy-loaded at send time)
```
...
>
> **Encryption keys**: generate both with `openssl rand -hex 32` — anything shorter throws at first encrypt/decrypt call. CI uses fixed hex fixtures (see `src/tests/setup.js`).
...
| `npm run db:push:LOCAL_ONLY` | Push schema directly, **local databases only** — no migration file, no audit gate. Renamed from `db:push` per ADR-078 / INC-005. Never run against staging or production; use `docs/runbooks/RUNBOOK-migration-baseline.md`. |
...
### Resend: "Missing API key" ตอน `npm run build`
`src/lib/email.js` ใช้ lazy singleton — error นี้เกิดเฉพาะตอน runtime (ส่งอีเมลจริง)
ถ้าเจอตอน build แสดงว่ามีคนเปลี่ยนกลับไป instantiate ที่ top-level — อย่า commit.

### `npm ci` FAIL ด้วย `Missing: <pkg> from lock file` ใน CI
เกิดเมื่อติดตั้ง dep ใหม่ด้วย `npm install` แล้วไม่ commit `package-lock.json` ทั้งหมด
แก้: `npm install --package-lock-only && git add package-lock.json`

...
| GET | `/api/integration/model-providers?businessId=` | Business owner (FR-266, ADR-100 D4); no write gate a read of whether a key exists must not itself require a step-up. Response `{modelCredential:{connectionId,provider,status,secretStore,lastValidatedAt,version} or null, providers}`; never material, and no `displayHint` field at all because a model key has no non-secret identifier (SDD-101). `no-store`. |
| POST | `/api/integration/model-providers` | Business owner; FR-223/FR-224 gate and limit; `{businessId,provider,apiKey}` proved live against the provider before anything is stored (422 `MODEL_KEY_REJECTED`, 503 `MODEL_PROVIDER_UNAVAILABLE`, 400 `MODEL_PROVIDER_UNSUPPORTED`), then written through the SecretStorePort as `MODEL_PROVIDER_KEY`. Entering a first key and rotating are the same call; a key for a different vendor replaces the connection's provider rather than creating a second one. Response `{connectionId,provider,providerChanged,credential}`; `no-store`. |
...
| POST | `/api/integration/model-providers/[id]/validate` | Business owner; empty body with the gate and limit; re-proves the stored key with the provider and records `lastValidatedAt`/`lastValidationCode` including on rejection, so the readiness journey stops reporting an older success. 409 `CREDENTIAL_REENTRY_REQUIRED` when it no longer resolves, 422 `MODEL_KEY_REJECTED`. Response `{credential,provider}`; `no-store`. |

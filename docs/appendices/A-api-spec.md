...
<!-- api-spec-counts: route_handlers=76 -->
...
its own fallback text, so only the sender knows which message the customer read
(BR-011). Recording at hand-over would record a message nobody received.

| Method | Path | Contract |
|---|---|---|
| POST | `/api/agent/line-delivery` | implemented: batch of delivery receipts, each naming the inbound `Message.id` it answers and the text actually sent, plus `source` (`STACK` \| `TRANSPORT_FALLBACK`) and an optional provider message id. Scope resolves through the same binding seam as the webhook (FR-052). Idempotent on `reply:<inboundMessageId>`; a per-receipt failure is reported with its stage and never drops the rest of the batch |
...
| POST | `/api/mcp` | MCP JSON-RPC transport for `initialize`, `notifications/initialized`, `tools/list`, and `tools/call`; resolves the trusted request viewer before any session or tool operation; requires `Mcp-Session-Id` after initialization (FR-069) |
...
- every current API route handler is represented by a current path in this
  appendix;
- the `route_handlers=68` marker matches the route-file enumeration;

...
<!-- api-spec-counts: route_handlers=72 -->
...
### Production Authentication & Password Reset (FR-081)

| Method | Path | Success | Failure |
|---|---|---|---|
| POST | `/api/auth/login` | `200 { success: true, user, redirect: "/businesses" }`; sets HttpOnly `zuri_session` cookie | `401 { error: "INVALID_CREDENTIALS" }` |
| POST | `/api/auth/logout` | `200 { success: true }`; clears `zuri_session` cookie | `500 { error: "LOGOUT_FAILED" }` |
| POST | `/api/auth/forgot-password` | `200 { success: true, message: "Reset instructions generated" }`; creates `PasswordResetToken` | `400 { error: "EMAIL_REQUIRED" }` |
| POST | `/api/auth/reset-password` | `200 { success: true, message: "Password updated successfully" }`; updates `PersonCredential` | `400 { error: "INVALID_OR_EXPIRED_TOKEN" }` |

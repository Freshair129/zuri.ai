...
<!-- api-spec-counts: route_handlers=83 -->
...
| POST | `/api/session/login` | sets an HttpOnly SameSite=Lax server-owned session cookie and redirects `303 /businesses`; local demo cookie is allowed only with explicit non-production capability | `500 { error: "FAILED" }` |

...
<!-- api-spec-counts: route_handlers=287 -->
...
| POST | `/api/crm/customers/[customerId]/chat-evidence/legal-hold` | implemented: SEC-034 archive legal hold (ADR-093 D6, TASK-ZAI-113) — a per-Business **owner** over a Business in the Customer's tenant (BR-001, same shape as the consent/erasure/retrieval routes above). Body `{ businessId, reason, endDate }` (`endDate` `YYYY-MM-DD`, must be in the future; `reason` required free text — a dispute reason, not validated against another registry). Rows accumulate per Customer rather than one mutable row a later hold overwrites; "does this Customer currently have a hold" is answered by `endDate > now` over every row. A hold defers `CustomerArchiveKey` destruction past a PDPA erasure (`erase-principal.js`) or the ADR-093 D5 term expiry (`chat-evidence-archive-expiry-service.js`) until it ends — see the erasure route's response `archiveKeyStatus` for how a hold is shown. Writes one `ARCHIVE_LEGAL_HOLD_RECORDED` audit event naming the Customer, the reason and the end date. `400` for an end date not in the future; `403` for a Business seen but not owned; `404` for an unknown Business or a Customer outside its tenant |
...
| PATCH | `/api/line-oa/accounts/[id]` | implemented (FR-146): `{ action, version, transportMode? }` applies one versioned action under publisher authority — `PAUSE` (CONNECTED → PAUSED), `RESUME` (PAUSED → CONNECTED), `ARCHIVE` (any non-terminal → ARCHIVED, clears the default), `SET_DEFAULT` (moves the Business's single default), `SWITCH_TRANSPORT_MODE` (EDGE ↔ CLOUD, records the count of jobs cancelled under the old owner — zero until the job lane exists). The update is a compare-and-swap on `(id, version)`; each action writes one audit row without secrets or customer content. There is no DELETE: archiving keeps the row | `404 Business not found`; `409 LINE_OA_ACCOUNT_VERSION_CONFLICT \| LINE_OA_ACCOUNT_TRANSITION_INVALID \| LINE_OA_ACCOUNT_ARCHIVED \| LINE_OA_ACCOUNT_ALREADY_DEFAULT \| LINE_OA_TRANSPORT_MODE_UNCHANGED`; `400` validation (`version` required; `transportMode` required for a switch) |
...
|---|---|---|---|---|---|
| 1.81.0b | 2026-09-16 | candidate | SEC-034 (ADR-093 D6, TASK-ZAI-113): one handler file, `POST /api/crm/customers/[customerId]/chat-evidence/legal-hold` — an OWNER records a dispute reason and an end date deferring a Customer's archive key destruction past a PDPA erasure or the D5 term expiry. Route handler count 286 -> 287 | working-tree | Claude Sonnet 5 |

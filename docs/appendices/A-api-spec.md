...
## Plugin authentication and capability discovery (FR-094 / ADR-045)

This is the canonical public-client boundary for first-party Codex/Claude Code
plugins. Authorization starts from the existing trusted browser session; the plugin
never submits a password, copies `zuri_session` or treats `Mcp-Session-Id` as a grant.
Client id and exact redirect URI allowlists are environment configuration and missing
configuration fails closed.

| Method | Path | Contract |
|---|---|---|
| GET | `/api/plugin/auth/authorize` | requires trusted browser session; validates exact `client_id`, redirect URI, PKCE S256 challenge, state and installation id; issues one hashed, short-lived code and redirects only to the configured URI |
| POST | `/api/plugin/auth/token` | exchanges `{grant_type: authorization_code, code, client_id, redirect_uri, code_verifier, installation_id}`; atomically consumes the code and returns a short-lived opaque bearer session with no refresh token |
| GET | `/api/plugin/auth/capabilities` | requires opaque bearer session; resolves Person through `resolveViewer`; returns bounded policy snapshot and server-derived capabilities only |
| POST | `/api/plugin/auth/revoke` | accepts `{token, token_type_hint?: access_token}`; hash-bound idempotent revoke with no token-existence disclosure |

Local implementation is verified by route/unit tests. Production client registration,
device proof-of-possession and live revocation/cross-tenant evidence remain gated.

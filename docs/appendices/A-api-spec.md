...
| POST | `/api/session/login` | sets an HttpOnly SameSite=Lax live owner session cookie and redirects `303 /businesses` | `500 { error: "FAILED" }` |
...
- every current API route handler is represented by a current path in this
  appendix;
- the `route_handlers=80` marker matches the route-file enumeration;

...
| ArchiveManifest | id UUID, tenantId (plain column), runId, filePath, fileSha256, messageCount, messageIdListHash, previousManifestId → ArchiveManifest (self, Restrict)?, previousManifestHash?, manifestHash, fileDeletedAt?, createdAt; `@@unique([tenantId, runId])` | FR-245 / SDD-103 / SEC-034 / ADR-093 D2, D4 — one row per archive file the retention sweep has written and verified, chained per Tenant (`manifestHash` covers the previous manifest's own hash). Inserted only in the same transaction that tombstones exactly the messages the file archived — a row here always names a real, intact, already-verified file. `fileDeletedAt` (TASK-ZAI-113) is set once the ADR-093 D5 expiry sweep deletes the file because every line in every segment it held has expired; the row itself is kept as the chain-of-custody record. **Included** in the backup snapshot: `chat-evidence-retrieval-service.js` finds archive files strictly by walking this table's rows per Tenant, so excluding it would make every archived message undiscoverable after a restore even when the files themselves (or their ADR-093 D8 offline copy) survive intact. The self-referential chain FK is the only real foreign key. Production DDL is an unapplied artifact (migration `20260916150000`; `fileDeletedAt` and `ArchiveLegalHold` below in `20260916190000`). |
| ArchiveLegalHold | id UUID, tenantId (plain column, no relation), customerId (plain column, no relation), reason, endDate, recordedByPersonId (plain column, no relation), createdAt | SEC-034 / ADR-093 D6 — an OWNER-recorded legal hold on one Customer's chat evidence archive: a dispute reason and an end date, deferring the archive key's destruction on PDPA erasure or term expiry (TASK-ZAI-113). Rows accumulate per Customer (never one mutable row a later hold overwrites); "does this Customer currently have a hold" is answered by `endDate > now` over every row. **Included** in the backup snapshot — unlike `CustomerArchiveKey`/`ArchiveManifest`, this row holds no ciphertext at all, and losing it on restore would leave a Customer's key looking eligible for destruction when an OWNER had specifically protected it. Production DDL is an unapplied artifact (migration `20260916190000`). |
...
Version diff 1.52.0b → 1.53.0b (2026-09-16): TASK-ZAI-113 (SEC-034, ADR-093 D5, D6) — new model `ArchiveLegalHold` (crm; included in the backup snapshot from the start, unlike the two rows above it, since this decision needed no reversal) and a new nullable column `ArchiveManifest.fileDeletedAt`. An OWNER-recorded hold (a reason and an end date) defers a Customer's `CustomerArchiveKey` destruction past a PDPA erasure or the D5 term-expiry sweep; the sweep destroys a key once no unexpired archived line remains anywhere for that Customer, and deletes a file once every segment in it has expired, setting `fileDeletedAt` rather than deleting the manifest row. 168 models are now declared. Migration `20260916190000_crm_archive_legal_hold` written, not applied.

...
| Product carton fields | `unitsPerCarton?`, `cartonCbm?`, `cartonKg?`, `freightGoodsType?` on `Product` | TASK-ZAI-053 — nullable Inventory-owned carton facts; `CARTON_DATA_MISSING` is reported for counted SKUs without the three physical measurements |
...
| SupplierCostSheet | code (unique per tenant), tenantId, businessId, supplierId → Supplier (Restrict), currency, fxRateLocked, sourceRef?, sourceSha256 (unique per Business), previewHash, previewJson, status (DRAFT / CONFIRMED / SUPERSEDED), lineCount, createdByPersonId?, confirmedByPersonId?, confirmedAt?, supersededAt?, version | TASK-ZAI-053 — one Procurement-owned factory source version; preview is idempotent on the source hash and stores no confirmed cost lines until a person confirms every mapping; locked FX is part of the immutable source snapshot |
| SupplierCostLine | sheetId → SupplierCostSheet (Cascade), productId → Product (Restrict), sourceSku, minQty, unitCostForeign, unitsPerCarton?, cartonCbm?, cartonKg?, freightGoodsType?, leadTimeDays?, mappingConfidence, mappingConfirmedByPersonId?, mappingConfirmedAt?; unique `(sheetId, sourceSku, minQty)` | TASK-ZAI-053 — one confirmed SKU price break; the read projection derives integer-satang and baht values from the sheet's locked FX; carton columns mirror the source while Product remains Inventory-owned |

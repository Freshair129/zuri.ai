...
| สองบันได | `GOODS_RECEIVER` (`procurement.receipt.post`) หรือ OWNER post ใบรับของ — `PROCUREMENT_BUYER` ถือแค่ `procurement.po.write` และทั้งสอง role ขัดกันตาม SoD (ADR-079 D2 amend ADR-066 D4); ส่วนที่เขียน ledger ต้องมีสิทธิ์เขียนของ Inventory ด้วย (`403 PROCUREMENT_RECEIPT_REQUIRES_INVENTORY_AUTHORITY`) — role ของ Procurement ไม่ขยาย Inventory (ADR-066 D4 = ADR-065 D4) |

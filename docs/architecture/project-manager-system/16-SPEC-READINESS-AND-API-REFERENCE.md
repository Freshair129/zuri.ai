...
| SPEC-G09 Registration and baseline | ADR-095 / FR-247 register only the candidate NAV slice; other PMR/PMF/PMD IDs remain proposal-local | Reconcile current global registry, allocate canonical IDs through the ledger, resolve peer/parent decisions and pin an approved per-slice baseline |

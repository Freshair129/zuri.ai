...
**Spec completeness:** [16 Spec readiness & API reference](16-SPEC-READINESS-AND-API-REFERENCE.md) ตรวจไฟล์จริงแล้ว: มี contracts / API schemas / OpenAPI references และเพิ่ม Swagger UI ใน local review แต่ยังต้องปิด SPEC-G01–G09 ก่อน spec-to-code. 125 schema definitions เป็น API/JSON shapes ไม่ใช่ตารางฐานข้อมูล; ห้ามตีความ structural validation ว่า implementation-ready.

ชุดเอกสารประกอบด้วย 21 Markdown documents, 30 diagrams, 33 requirements/acceptance families และ OpenAPI candidates 80 operations (72 original + 8 workforce) พร้อม workflow schema/example, architecture/traceability, navigation และ UX/UI models; UX/UI supplement มี 37 screen families, 10 journeys, 13 forms / 83 fields และ 14 UI acceptance scenarios; เพิ่ม semantic audit ของ 9 Project sections, 14 routes และ 7 Work views ก่อนจัดเมนูใหม่
...
**Navigation review state:** ถอนข้อเสนอ NAV-D1/D2 ที่เปลี่ยน sidebar เป็นทุก feature ของ Project และลบ ProjectTabs; `navigation.candidate.json`, screen mappings และ HTML v0.3 ยังเป็น snapshot ของข้อเสนอเก่า ต้อง reconcile ตามเอกสาร 13 ก่อนใช้ spec-to-code หรืออนุมัติ NAV-P1 ข้อกำหนดข้อมูล/API/run ไม่ถูกเปลี่ยนด้วยการจัดเมนูนี้
...
**Multi-agent delivery:** [20 Delivery plan](20-MULTI-AGENT-DELIVERY-PLAN.md) กำหนด Luna Max workers 2 ตัว, independent Luna Max verifier 1 ตัว และ root เป็น final gate/integrator; แยก core NAV กับ Workforce bindings และ release ของ Workforce จาก Fleet. เป็นแผนงาน ยังไม่ dispatch implementation.

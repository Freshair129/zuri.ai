...
- **Follows:** SDD-018, SDD-024, SDD-052, SEC-008
- **Tests:** `tests/e2e/fr046-entry-contract.spec.js` · `tests/e2e/smoke.spec.js` · `tests/unit/fr046-api-ui-contract.test.js` · `tests/unit/platform-control-route-contract.test.js` · `tests/unit/scope-view-context.test.js`
...
- **Tests:** `tests/e2e/fr041-business-first.spec.js` · `tests/e2e/fr059-strategy-edit.spec.js` · `tests/integration/fr059-business-strategy-mutation.test.js` · `tests/unit/fr045-api-ui-contract.test.js` · `tests/unit/fr059-strategy-edit-ui.test.js` · `tests/unit/fr059-strategy-validation.test.js` · `tests/unit/fr060-business-home-read-model.test.js` · `tests/unit/fr060-business-home-visibility.test.js` · `tests/unit/overview-split.test.js` · `tests/unit/repositories-page-ui-contract.test.js` · `tests/unit/viewer-authority.test.js`
...
- **Follows:** BR-001, BR-011, SDD-007, SDD-018, SDD-050, SEC-001, SEC-009
- **Tests:** `tests/e2e/fr041-business-first.spec.js` · `tests/e2e/fr060-business-home.spec.js` · `tests/e2e/fr091-conversation-inbox.spec.js` · `tests/integration/crm-conversation-inbox.test.js` · `tests/integration/line-reply-record.test.js` · `tests/unit/command-palette-index.test.js` · `tests/unit/conversation-read-model.test.js` · `tests/unit/domain-navigation.test.js` · `tests/unit/fr045-api-ui-contract.test.js` · `tests/unit/fr091-inbox-ui-contract.test.js`
...
### FR-094 — Platform Programme Roadmap: `/control/roadmap` is an installation-operator-only, read-only projection of the submitted 24-week programme. It is deliberately outside BusinessShell and `DOMAINS`: there is no active Business, Tenant, Project or Workstream scope, and the surface must not be mistaken for a user's operational work. A missing viewer follows the entry boundary; a trusted viewer is admitted only by the named `isOperator` capability from FR-075 — never by `isPlatform`, role, Business/Tenant ownership or domain visibility. The board has a static plan snapshot (six phases, twelve sprints, thirty tasks, eight gates and ten deliverables) and a separately labelled build-time evidence snapshot. Day 1 is GitHub repository creation (2026-08-11); aggregate default-branch history and verified SmartGift migration facts are evidence, never programme completion. `/programme` is the separately approved public, removable share of that exact safe aggregate; it does not authorize `/control/roadmap`. Neither route exposes PII, raw rows, prices, cost, credentials, local paths, API or a write path.

- **Status:** n/a
- **Surface:** `/control/roadmap` (page) · `/programme` (page)
- **Code:** `src/app/(control)/control/roadmap/page.jsx` · `src/app/(control)/layout.jsx` · `src/app/programme/page.jsx` · `src/components/layouts/PlatformControlGuard.jsx` · `src/components/layouts/PlatformControlShell.jsx` · `src/context/ScopeContext.jsx` · `src/lib/platform-control-guard.js` · `src/modules/platform-control/components/ProgramRoadmapBoard.jsx` · `src/modules/platform-control/program-roadmap-data.js`
- **Follows:** NFR-008, SDD-018, SDD-024, SDD-052, SEC-008, SEC-018
- **Tests:** `tests/e2e/fr046-entry-contract.spec.js` · `tests/e2e/smoke.spec.js` · `tests/unit/fr046-api-ui-contract.test.js` · `tests/unit/platform-control-guard.test.js` · `tests/unit/platform-control-route-contract.test.js` · `tests/unit/scope-view-context.test.js`

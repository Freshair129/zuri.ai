...
**ยังเปิดอยู่หลัง wave 2:** ~~apply migration `line-oa` → `LINE_OA` บน production~~ (apply แล้ว 2026-09-03 — ledger ใน `docs/runbooks/line-oa-provider-merge.md` §5; SQL Step 2 ถูกแก้หลัง rollback 42P01 และ read tolerance ถูกถอดออก), e2e สำหรับ panel ใหม่บน `/platform/users` และปุ่ม erasure บน inbox, reactivate Membership ที่ REVOKED, `tests/factories/viewer.js` DEFAULT_DOMAINS ล้าสมัย, session `task-modal-envelope` ของอีก session ขัดกับการตัดสินใจ FR-017 (quick-add ไม่ต้องเข้า envelope) — ควรยกเลิก

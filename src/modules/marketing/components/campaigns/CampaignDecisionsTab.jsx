    ...
    <div className="space-y-4" data-testid="marketing-campaign-decisions">
      <Card>
        ...
        {!plan ? <UnavailableState title="Strategy evidence unavailable" hint="The linked Strategy plan cannot be read in this Business scope." /> : <>
          ...
          {(plan.reviews || []).length > 0 && <section><h3 className="mb-2 text-[11px] font-bold">Review history</h3><ul className="space-y-2">{plan.reviews.map((review) => <li key={review.id} className="rounded-lg border border-[var(--border)] p-2 text-[10px]"><div className="flex flex-wrap items-center gap-2"><StatusPill status={review.verdict} /><span className="text-muted">{formatMarketingDate(review.createdAt)}</span></div><p className="mt-1">{review.rationale || 'No rationale recorded.'}</p></li>)}</ul></section>}
          {(plan.decisions || []).length > 0 && <section className="mt-4"><h3 className="mb-2 text-[11px] font-bold">Decision history</h3><ul className="space-y-2">{plan.decisions.map((decision) => <li key={decision.id} className="rounded-lg border border-[var(--border)] p-2 text-[10px]"><div className="flex flex-wrap items-center gap-2"><StatusPill status={decision.verdict} /><span className="text-muted">{formatMarketingDate(decision.createdAt)}</span></div><p className="mt-1">{decision.rationale || 'No rationale recorded.'}</p></li>)}</ul></section>}
        </>}
      </Card>
      ...
      {(campaign.status === 'CLOSED' || campaign.status === 'CANCELLED') && <InlineNotice>{campaign.status === 'CLOSED' ? 'This Campaign is closed and read-only.' : 'This Campaign is cancelled and read-only.'}</InlineNotice>}
    </div>
  )
}

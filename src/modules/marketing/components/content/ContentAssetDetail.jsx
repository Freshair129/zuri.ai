...
export default function ContentAssetDetail({ businessId, assetVersionId }) {
  ...
  const source = brief?.references?.asset || null
  ...
  return <main className="mx-auto max-w-6xl p-5 max-md:p-3">
    <PageHeader eyebrow="MARKETING / CONTENT & CREATIVE" title={`${brief?.title || 'Creative asset'} · revision ${version.revision || 'unavailable'}`} subtitle={`Owner ${contentOwnerLabel(version)} · source and rights remain Files-owned`} actions={<StatusPill status={usable ? 'APPROVED' : current ? 'REVIEW' : 'ARCHIVED'} />} />
    ...
    <MarketingDataState loading={detail.loading && !data} error={null} retry={detail.reload}>
      <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
        <Card warm data-testid="marketing-content-asset-source"><SectionTitle caption="Files owns bytes and serves them through its own authorization gate">Creative source</SectionTitle><div className="grid min-h-48 place-items-center rounded-xl border border-dashed border-[var(--border)] bg-[var(--surface-mid)] p-6 text-center"><p className="text-sm font-semibold">Preview is available in Files</p><p className="mt-1 max-w-md text-xs text-muted">Content shows the authorized source reference; it does not fabricate a creative preview or copy file bytes.</p>{fileLink && <a className="btn mt-3 inline-flex items-center gap-1 text-[11px]" href={fileLink}><ExternalLink size={13} aria-hidden /> Open in Files</a>}{!fileLink && <p className="mt-3 text-[10px] text-muted">Files source unavailable for this revision.</p>}</div></Card>

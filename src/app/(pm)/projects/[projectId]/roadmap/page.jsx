...
function WorkItemRow({ item }) {
  return (
    <div className="rounded-lg border border-[var(--border)] bg-[var(--surface-card)] px-3 py-2">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="text-[10px] text-muted">{item.code} · {item.subtype}</p>
      ...
      <div className="mt-3 space-y-2">
        {planContainers.map((container) => (
          <div key={container.containerId} className="rounded-lg bg-[var(--surface-mid)] px-3 py-2">
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="text-[10px] text-muted">{container.code} · {container.subtype}</p>
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 max-lg:grid-cols-1">
        <Card>
          ...
          <div aria-label="Roadmap dependency list" className="space-y-2">
            {data.dependencies.items.length === 0 ? <p className="text-xs text-muted">No project-contained dependencies.</p> : data.dependencies.items.map((edge) => (
              <div key={edge.id} className="rounded-lg bg-[var(--surface-mid)] px-3 py-2 text-xs">
                ...
                <span className="ml-2 text-muted">{edge.dependencyType}</span>
              </div>
            ))}
          </div>
        </Card>
        <Card>
          ...
          <div className="mt-3 space-y-1 text-xs">
            {data.closure.gates.length === 0 ? <p className="text-muted">No gates recorded.</p> : data.closure.gates.map((gate) => <p key={gate.id}>{gate.code} · {gate.title} · {gate.status}</p>)}
          </div>
        </Card>
      </div>

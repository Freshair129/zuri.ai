...
// @tested tests/e2e/fr077-project-inventory.spec.js, tests/unit/project-inventory-ui.test.js
...
/**
 * A card's header caption has to describe what the card's *body* actually
 * renders, not just what the read model returned.
 *
 * `collection` already carries an honest count (`items.length`) against its
 * own pagination window (SDD-045's `paginateCollection`) — that part never
 * lied. What could lie is a *second*, purely visual truncation: a card that
 * previews only `previewCount` rows of `collection.items` for compactness.
 * Left unaccounted, the header keeps reporting the full collection count while
 * the body quietly shows fewer rows — "7 shown" over 5 rendered rows, with no
 * way to reach the other 2. `previewCount` makes that second cut visible in
 * the same caption, and `moreHref` gives it the way out the sibling Work
 * items card already has (FR-077).
 *
 * Pure and exported so the regression (header count and rendered rows parting
 * company) is pinned by a direct unit test rather than only by reading JSX.
 */
export function inventorySectionCaption(total, previewCount) {
  const displayTruncated = previewCount != null && previewCount < total
  return {
    displayTruncated,
    caption: displayTruncated ? `${previewCount} of ${total} shown` : `${total} shown`,
  }
}

function InventorySection({ title, collection, noun, previewCount, moreHref, moreLabel, children }) {
  const total = collection.items.length
  const { displayTruncated, caption } = inventorySectionCaption(total, previewCount)
  return (
    <Card>
      ...
      {total > 0 && children}
      {displayTruncated && moreHref && (
        <Link className="mt-3 inline-block text-[11px] font-semibold text-brand-dark" href={moreHref}>
          {moreLabel || `Open all ${noun} →`}
        </Link>
      )}
    </Card>
  )
  ...
  return (
    <div>
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 max-lg:grid-cols-1">
        <Card>
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="shrink-0 text-sm font-bold">{formatProgressPercent(sections.progress.rollup.percent)}</span>
          </div>
        ...
        <InventorySection
          title="Workstreams"
          collection={sections.work.workstreams}
          noun="workstreams"
          previewCount={5}
          moreHref={`/projects/${project.id}`}
          moreLabel="Open all workstreams →"
        >
      ...
      <div className="grid grid-cols-2 gap-3 max-lg:grid-cols-1">
        <InventorySection title="Work items" collection={sections.work.items} noun="work items" previewCount={6}>
          <div className="space-y-2">
            {workItems.map((item) => (
        ...
        <InventorySection title="Milestones" collection={sections.milestones} noun="milestones" previewCount={5}>
          <div className="space-y-2">
        ...
        <InventorySection title="Gates" collection={sections.gates} noun="gates" previewCount={5}>
          <div className="space-y-2">
        ...
        <InventorySection title="Dependencies" collection={sections.dependencies} noun="dependencies" previewCount={5}>
        ...
        <InventorySection title="Files" collection={sections.files} noun="files" previewCount={5}>
          <div className="space-y-2 text-xs">
        ...
        <InventorySection title="Linked repositories" collection={sections.repositories} noun="repositories" previewCount={5}>
          <div className="space-y-2 text-xs">
        ...
        <InventorySection title="Project team" collection={sections.team} noun="team members" previewCount={5}>
          <div className="space-y-2 text-xs">
        ...
        <InventorySection title="Recent activity" collection={sections.activity} noun="activity events" previewCount={5}>
          <div className="space-y-2 text-xs">

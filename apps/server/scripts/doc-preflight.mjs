#!/usr/bin/env node
import { workspacePath, workspaceRoot, canonicalRelative } from './workspace-path.mjs'
...
const REPORT = workspacePath(ROOT, 'docs', '.preflight-report.json')
const GRAPH = workspacePath(ROOT, 'docs', '.doc-graph.json')
...
const rel = (p) => canonicalRelative(ROOT, p)
const findings = []
...
if (!existsSync(GRAPH)) {
  ...
  {
    const MODULES_DIR = workspacePath(ROOT, 'src', 'modules')
...
const routeFiles = walk(workspacePath(ROOT, 'src', 'app', 'api'), 'route.js')
...
const apiSpec = workspacePath(ROOT, 'docs', 'appendices', 'A-api-spec.md')
...
const pageFiles = walk(workspacePath(ROOT, 'src', 'app'), 'page.jsx')
const pageRoutes = pageFiles.map(appRoute).sort()
const interfaceInventory = workspacePath(ROOT, 'docs', 'INTERFACE-INVENTORY.md')
if (existsSync(interfaceInventory)) {
  ...
  if (!declared) {
    ...
    const domainConfig = read(workspacePath(ROOT, 'src', 'config', 'domains.js'))
...
const schema = workspacePath(ROOT, 'prisma', 'schema.prisma')
const dbSpec = workspacePath(ROOT, 'docs', 'appendices', 'B-db-schema.md')
...
const testCount = walk(workspacePath(ROOT, 'tests'), '.test.js').length + walk(workspacePath(ROOT, 'tests'), '.spec.js').length
...
])
{
  ...
  const offenders = []
  for (const file of walk(workspacePath(ROOT, 'tests'), '.js')) {
...
const ENUM_SOURCE = workspacePath(ROOT, 'src', 'lib', 'validation', 'enums.js')
if (existsSync(ENUM_SOURCE)) {
  ...
  const WINDOW = 5
  const offenders = []
  for (const file of walk(workspacePath(ROOT, 'src'), '.js').concat(walk(workspacePath(ROOT, 'src'), '.jsx'))) {
...
const BACKUP_SERVICE = workspacePath(ROOT, 'src', 'modules', 'project-manager', 'application', 'backup-service.js')
const PRISMA_SCHEMA = workspacePath(ROOT, 'prisma', 'schema.prisma')
...
const ROUTE_VIEWER_BASELINE = path.join(SPEC_PACK, '.route-viewer-baseline.json')
{
  ...
  const IS_EDGE_DEVICE_ENDPOINT = (p) =>
    ...
    // FR-265/ADR-100 D2: the five conversation-job paths and the residency poll
    // that used to be exempted after this line are withdrawn. Their exemptions go
    // with them — an allowance for a route that no longer exists is an allowance
    // waiting to be inherited by whatever is created at that path next.
...
{
  const offenders = []
  for (const file of walk(workspacePath(ROOT, 'src'), '.jsx')) {
    const body = read(file)
...
const ID_LEDGER = path.join(SPEC_PACK, '.id-ledger.json')
{
  ...
  const readDoc = (repoRelPath) => {
    const abs = workspacePath(ROOT, repoRelPath)
...
if (existsSync(ROADMAP)) {
  const { broken, structural } = findBrokenEvidence({
    ...
    exists: (repoRelPath) => existsSync(workspacePath(ROOT, repoRelPath)),
  })
...
{
  const runGit = () => {
    const r = spawnSync('git', GIT_ARGS, { cwd: workspaceRoot(ROOT), encoding: 'utf8', windowsHide: true })
...
// reader gets this check's actual reach rather than the reach they assume.
{
  ...
  if (!existsSync(LEDGER)) {
    ...
    const listMarkdown = (dir) =>
      walk(workspacePath(ROOT, dir), '.md')
    ...
    for (const f of evaluateTableIntegrity({
      ...
      read: (p) => read(workspacePath(ROOT, p)),
      exists: (p) => existsSync(workspacePath(ROOT, p)),
      listMarkdown,
    })) {
      add(f.severity, f.check, f.title, f.details, f.files, f.action)
    }
...
{
  const postgresSchema = workspacePath(ROOT, 'prisma', 'schema.postgres.prisma')
  const migrationsDir = workspacePath(ROOT, 'supabase', 'migrations')

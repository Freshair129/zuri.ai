...
const rel = (root, file) => canonicalRelative(root, file)
...
function runtimeContractCheck(root, requirements) {
  ...
  const hasPlanRuntime = existsSync(workspacePath(root, planSchema))
...
function jsonSchemaCheck(root, featureDocs) {
  ...
  const schemaPath = workspacePath(root, 'contracts', 'plan-envelope.schema.json')
...
function featureDocsForDomain(root, domainName) {
  const dir = workspacePath(root, 'docs', 'domains', domainName, 'features')
...
export function discoverFeatureRequirements(root) {
  ...
  for (const domainDir of readdirSync(workspacePath(root, 'docs', 'domains'))) {

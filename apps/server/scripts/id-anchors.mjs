...
function findByBasename(root, relPath) {
  ...
  const docs = workspacePath(root, 'docs')
  const found = []
  const walk = (dir) => {
...
export function resolveRegistry(root, reg) {
  if (reg.file) {
    if (existsSync(workspacePath(root, reg.file))) return reg.file
    return findByBasename(root, reg.file)
  }
  if (existsSync(workspacePath(root, reg.dir))) return reg.dir
  return null
}

function collectTable(root, reg, file, out, dups) {
  const idRe = tableIdRe(reg.families)
  for (const line of readCanonical(workspacePath(root, file)).split('\n')) {
...
function collectDocumentH1(root, reg, dir, out, dups) {
  ...
  for (const base of readdirSync(workspacePath(root, dir)).sort()) {
    if (!base.endsWith('.md')) continue
    const m = namePattern.exec(base)
    if (!m) continue
    const id = `${family}-${m[1]}`
    const body = readCanonical(workspacePath(root, dir, base))

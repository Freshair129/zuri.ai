...
function walk(dir, exts, out = []) {
  ...
  for (const entry of readdirSync(dir)) {
    const canonicalDocs = dir === SPEC_PACK || dir.startsWith(SPEC_PACK + path.sep)
    if (SKIP_DIRS.has(entry) && !(canonicalDocs && entry === 'migrations')) continue
...
function build() {
  ...
  const appFiles = walk(workspacePath(ROOT, 'src', 'app'), ['.jsx', '.js'])
...
function featureMap(nodes, edges) {
  ...
  const featureNoteFiles = [
    ...walk(workspacePath(ROOT, 'docs', 'domains'), ['.md']).filter((f) => f.includes(`${path.sep}features${path.sep}`)),
    ...walk(workspacePath(ROOT, 'docs', 'features'), ['.md']),
  ]
  ...
  for (const f of walk(workspacePath(ROOT, 'docs', 'roadmap'), ['.md'])) {
...
// Keep historical Server node IDs stable while publishing real monorepo file locations.
if (workspaceRoot(ROOT) !== ROOT) {
  for (const node of nodes) {
    if (node.path && /^(?:src|tests|prisma)\//.test(node.path)) node.path = `apps/server/${node.path}`
  }
}

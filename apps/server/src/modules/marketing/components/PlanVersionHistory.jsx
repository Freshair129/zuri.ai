...
function displayValue(value, field) {
  if (field === 'campaignBrief' && value && typeof value === 'object') {
    return `Dates: ${value.startDate || '—'} → ${value.endDate || '—'} · Offer: ${value.offer || '—'} · Conditions: ${value.conditions || '—'}`
  }
  if (Array.isArray(value)) return value.map((item) => item?.title || item).join(', ')
    ...
    <Card className="mb-4" data-testid="marketing-plan-versions">
      ...
      <div className="space-y-2">
        {versions.map((version, index) => {
          ...
          if (currentPayload.campaignBrief) {
            content.push(['Campaign dates', `${currentPayload.campaignBrief.startDate || '—'} → ${currentPayload.campaignBrief.endDate || '—'}`])
            content.push(['Offer', currentPayload.campaignBrief.offer])
            content.push(['Conditions', currentPayload.campaignBrief.conditions])
          }
          return (

...
export const PROGRAMME_TASKS = [
  ...
  ['TASK-ZAI-073', 'SPR-ZAI-02', 'Zuri harness plugin specification — one document for the CLI, hook input, configuration and queue formats, pairing state machine, counting rules, keep-or-drop rules per response, Codex wrapper, log-format compatibility, versioning and privacy, each rule linked to the test that proves it', 'NFR', 'C-2', 'H2', 'review'],
  ...
  ['TASK-ZAI-075', 'SPR-ZAI-02', 'Usage detail on the programme board — phase and task telemetry show input, output, thinking and cache tokens separately, tool calls with the most used tools and the error rate, prompts and compactions, per lane, person and device', 'FR', 'C-2', 'H2', 'review'],
]

...
export async function verifyFr054DisposableClusterMarker(client, expectedMarker) {
  const { rows } = await client.query(
    "select current_setting('zuri.fr054_disposable_cluster', true) as marker",
  )
  if (!MARKER_PATTERN.test(expectedMarker ?? '') || rows?.[0]?.marker !== expectedMarker) {
    throw new Error('RUNTIME_ISOLATION_TEST_CLUSTER_MARKER_MISMATCH')
  }
}

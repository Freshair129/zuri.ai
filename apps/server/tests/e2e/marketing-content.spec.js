...
test.describe('Marketing Content first functional slice', () => {
  test('keeps all six interfaces addressable and supports real review to Library asset detail', async ({ page, browser }, testInfo) => {
    ...
    try {
      const signup = await secondaryContext.request.post('/api/auth/signup', { headers: { 'x-forwarded-for': '192.0.2.253' }, data: { displayName: 'Content reviewer', email: secondaryEmail, password: secondaryPassword } })

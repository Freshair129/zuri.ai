...
test.beforeAll(async () => {
  ...
  const category = await prisma.inventoryCategory.create({ data: { code: `CAT-${tag}`, nameTh: 'สินค้าตรวจนับ', tenantId: tenant.id, businessId: business.id } })
  const master = await prisma.productMaster.create({ data: { code: `MASTER-${tag}`, nameTh: 'สินค้าตรวจนับ', tenantId: tenant.id, businessId: business.id, categoryId: category.id } })
...
test('stocktake previews NONE/LOT, rejects stale counts, and retries one committed operation after response loss', async ({ page }) => {
  ...
  await expect(page.getByRole('alert')).toContainText('ยอดสต็อกเปลี่ยนแล้ว')
  await expect(page.getByRole('button', { name: 'ยืนยันบันทึกยอดนับ', exact: true })).toBeDisabled()
  await page.getByRole('button', { name: 'ตรวจสอบยอดก่อนบันทึก', exact: true }).click()
  await expect(page.getByRole('button', { name: 'ยืนยันบันทึกยอดนับ', exact: true })).toBeEnabled()
  const keys = []
  ...
  await page.getByRole('button', { name: 'ยืนยันบันทึกยอดนับ', exact: true }).click()
  await expect(page.getByRole('alert')).toContainText('ยังยืนยันการบันทึกไม่ได้')
  await page.getByRole('button', { name: 'ยืนยันบันทึกยอดนับ', exact: true }).click()
  await expect(page.getByRole('status')).toContainText('บันทึกยอดนับแล้ว')

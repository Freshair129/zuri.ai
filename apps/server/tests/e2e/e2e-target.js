...
function isPrimaryCheckout(root = ROOT) {
  try {
    ...
    const checkout = path.basename(root) === 'server' && path.basename(path.dirname(root)) === 'apps'
      && existsSync(path.join(candidate, 'monorepo.json')) ? candidate : root
    return statSync(path.join(checkout, '.git')).isDirectory()
  } catch {

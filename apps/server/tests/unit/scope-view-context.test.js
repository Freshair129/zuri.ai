// @req FR-039 — Group, Organization, and Business are the shell context.
// @spec SDD-018, ADR-011, ADR-076
...
describe('Base Context Bar', () => {
  it('maps only the three approved schema entities in the approved order', () => {
    ...
    expect(BASE_CONTEXT_LEVELS.map((level) => level.label)).toEqual(['Group', 'Organization', 'Business'])
  })
})

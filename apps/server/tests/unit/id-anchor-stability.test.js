...
const LEDGER_PATH = workspacePath(ROOT, 'docs', '.id-ledger.json')
...
describe('the committed ledger describes the tree it was generated from', () => {
  ...
  it('raises nothing critical against the live tree', () => {
    const findings = evaluateIdStability({
      declared,
      ledger,
      readDoc: (p) => (existsSync(workspacePath(ROOT, p)) ? readCanonical(workspacePath(ROOT, p)) : null),
    })
...
describe('the writer refuses to launder a repurpose', () => {
  const run = (args) => {
    try {
      return { code: 0, out: execFileSync(process.execPath, [workspacePath(ROOT, 'scripts', 'id-ledger.mjs'), ...args], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] }) }
    } catch (e) {

...
describe('FR-218 recording', () => {
  it('stores an unattributed deployment report and audits it without content', async () => {
    const { db, rows, audits } = fakeDb()
    const result = await recordProgrammeUsageReport(db, body({ aiAccount: 'claude-max-team-a', repository: 'Freshair129/zuri.ai' }), { knownTaskCodes: known })
    expect(result.status).toBe(201)
    expect(rows[0]).toMatchObject({ personId: null, installationId: null, branch: 'feat/usage-report', taskCode: null, aiAccountLabel: 'claude-max-team-a', repository: 'Freshair129/zuri.ai' })
    expect(audits[0]).toMatchObject({ entityType: 'PROGRAMME_USAGE_REPORT', action: 'REPORTED', actorType: 'AGENT', actorId: `codex:${body().sessionId}` })
    expect(JSON.parse(audits[0].payloadJson)).toMatchObject({ reporter: 'deployment', tokensUsed: 2000, cacheReadTokens: 90000 })
  })
  ...
  it('answers the same payload with a replay, and a resumed session whose counts only grow with an extension', async () => {
    const first = body()
    const replay = await recordProgrammeUsageReport(fakeDb({ existing: storedFrom(first) }).db, body({ endedAt: '2026-09-13T17:40:00+07:00' }), { knownTaskCodes: known })
    ...
    const extended = await recordProgrammeUsageReport(db, resumed, { knownTaskCodes: known })
  ...
  it('conflicts when a count shrinks, the start moves, or another installation claims the key', async () => {
    const first = body()
    const cases = [
      [body({ outputTokens: 700 }), storedFrom(first)],
      [body({ outputTokens: 900, startedAt: '2026-09-13T09:00:00.000Z' }), storedFrom(first)],
    ]
    for (const [next] of cases) {
      const r = await recordProgrammeUsageReport(fakeDb({ existing: storedFrom(first) }).db, next, { knownTaskCodes: known })
      expect(r).toMatchObject({ status: 409, body: { error: 'USAGE_REPORT_CONFLICT' } })
    }
    const historical = storedFrom(first, { personId: 'per-1', installationId: 'inst-1' })
    const fromDeployment = await recordProgrammeUsageReport(fakeDb({ existing: historical }).db, body({ outputTokens: 900 }), { knownTaskCodes: known })
    expect(fromDeployment).toMatchObject({ status: 409, body: { error: 'USAGE_REPORT_CONFLICT' } })
  })
  ...
  it('treats a second branch of one session as its own report', async () => {
    ...
    const r = await recordProgrammeUsageReport(db, body({ branch: 'docs/usage-report-plan' }), { knownTaskCodes: known })
    expect(r.status).toBe(201)
  })

  it('resolves a lost race against the winner', async () => {
    const winner = storedFrom(body(), { id: 'row-2' })
    expect((await recordProgrammeUsageReport(fakeDb({ raceWinner: winner }).db, body(), { knownTaskCodes: known })).status).toBe(200)
  })
  ...
  it('refuses an unknown task and an invalid body by name', async () => {
    ...
    for (const bad of [body({ inputTokens: -1 }), body({ source: 'Codex CLI' }), body({ endedAt: '2026-09-13T09:00:00.000Z' }), { ...body(), prompt: 'secret text' }, body({ personId: 'per-forged' }), body({ installationId: 'inst-forged' }), body({ requestCount: 0 }), body({ branch: 'feat bad branch' })]) {
...
describe('FR-218 / FR-221 route and persistence contract', () => {
  it('checks the deployment credential before reading the body and never resolves a browser viewer', () => {
    ...
    expect(route.indexOf('bearerMatches(')).toBeGreaterThan(-1)
    expect(route.indexOf('bearerMatches(')).toBeLessThan(route.indexOf('request.json('))
    expect(route).not.toMatch(/HarnessCredential|authenticateHarnessCredential|HARNESS_NOT_ACTIVATED/)

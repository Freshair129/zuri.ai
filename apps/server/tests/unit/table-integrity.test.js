...
describe('scopeFromLedger — scope is read and swept, not hardcoded', () => {
  ...
  it('reads this repository own ledger and its real spec pack', () => {
    const listMarkdown = (dir, out = []) => {
      for (const e of fs.readdirSync(workspacePath(ROOT, dir), { withFileTypes: true })) {
        if (e.isDirectory()) listMarkdown(`${dir}/${e.name}`, out)
        else if (e.name.endsWith('.md')) out.push(`${dir}/${e.name}`)
      }
      return out
    }
    const scope = scopeFromLedger(fs.readFileSync(workspacePath(ROOT, 'docs', '.id-ledger.json'), 'utf8'), {
    ...
    for (const f of scope.files) expect(fs.existsSync(workspacePath(ROOT, f))).toBe(true)
  })
})
...
describe('the real document, not a fixture', () => {
  ...
  const REGISTRY = workspacePath(ROOT, 'docs', 'PRD-SDD-v1.0.md')
  ...
  it('pins the sitemap repair: a delimiter row that unmakes its own table', () => {
    ...
    const nav = fs
      .readFileSync(workspacePath(ROOT, 'docs', 'SITEMAP-DOMAIN-NAV.md'), 'utf8')
      .replace(/\r\n/g, '\n')
  ...
  it('pins the API spec repair: an unescaped pipe in prose truncates a row too', () => {
    ...
    const spec = fs
      .readFileSync(workspacePath(ROOT, 'docs', 'appendices', 'A-api-spec.md'), 'utf8')
      .replace(/\r\n/g, '\n')
  ...
  it('pins the appendix repair: the API spec Scope table is one contiguous table', () => {
    ...
    const spec = fs.readFileSync(workspacePath(ROOT, 'docs', 'appendices', 'A-api-spec.md'), 'utf8').replace(/\r\n/g, '\n')

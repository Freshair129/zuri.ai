...
// @req FR-079 — local/test secret adapter stores ciphertext only.
// @spec ADR-031 §D3, SEC-015
// @tested tests/unit/fr079-credential-vault.test.js

describe('FR-079 local credential vault', () => {
  let tempDir
  afterEach(async () => {
  ...
  it('round-trips a local secret while persisting ciphertext only', async () => {
    tempDir = await mkdtemp(path.join(os.tmpdir(), 'zuri-fr079-vault-'))

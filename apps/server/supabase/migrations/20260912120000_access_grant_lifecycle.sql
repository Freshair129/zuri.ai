...
-- ---------------------------------------------------------------------------
-- `crm` is a DOMAIN_GROUPS key, not a DOMAINS key. `buildDomainsByBusiness`
-- filters it out at read time, so the row reads as a grant and resolves to zero
-- domains, silently. Repaired to the group's children — which is what "give
-- them CRM" meant — and recorded either way.
DO $$
...
BEGIN
  ...
  LOOP
    UPDATE "Membership"
       SET "domainKeysJson" = (
         SELECT coalesce(jsonb_agg(DISTINCT k), '[]'::jsonb)::text FROM (
           SELECT CASE WHEN e = 'crm' THEN unnest(ARRAY['customer','market'])
                       WHEN e = 'scm' THEN unnest(ARRAY['inventory','warehouse','procurement','commerce'])
                       ELSE e END AS k
             FROM jsonb_array_elements_text(r."domainKeysJson"::jsonb) e
         ) x
         WHERE k IN ('business-home','commerce','customer','market','growth','operations',
    ...
    VALUES (gen_random_uuid()::text, 'MEMBERSHIP', r.id, 'DOMAIN_KEYS_REPAIRED',
            json_build_object('from', r."domainKeysJson", 'reason',
              'group key stored as a grant; expanded to its children (ADR-077, FR-192)')::text,
            'PIPELINE_OPERATOR', NULL, now());
    repaired := repaired + 1;
  END LOOP;
  RAISE NOTICE 'domainKeysJson repaired on % row(s)', repaired;
END $$;

-- @req FR-076, FR-078 — provision the application-side Prisma/Postgres schema
-- required for trusted viewer resolution and Business-scoped review RBAC.
-- @spec ADR-033 D8, ADR-018; the private customer data remains in zuri_core.
-- @tested tests/unit/application-schema-migration.test.js

-- Generated from prisma/postgres/0001_init.sql. Keep the canonical model in
-- prisma/schema.prisma and regenerate that source before changing this file.
...
-- Application tables are server-owned. They are deliberately not exposed to
-- anon/authenticated through the Supabase Data API; the Next.js server uses
-- the application database connection and enforces viewer/RBAC in code.
DO $$
DECLARE
  table_record record;
BEGIN

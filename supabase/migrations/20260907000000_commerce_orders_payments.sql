-- @req FR-158 — SalesOrder and SalesOrderLine: what a Business sold — lines
...
-- @req FR-159 — Payment: a payment or refund against an order, PENDING until a
...
COMMENT ON TABLE "SalesOrder" IS 'FR-158 — a sales order (order_id): Business-scoped, optional Customer/Conversation of the same Tenant, money in satang; totals and payment state computed on read (ADR-065).';
COMMENT ON TABLE "SalesOrderLine" IS 'FR-158 — one line of a sales order; may name an inventory SKU; qty × unitPriceSatang − discountSatang.';
COMMENT ON TABLE "Payment" IS 'FR-159 — a payment or refund against an order, PENDING until verified; bankReference unique per Tenant is an attribute (BR-002); the slip is a FileAsset.';

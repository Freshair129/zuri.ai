...
const ROUTES = [
  ...
  route('POST', '/v1/inventory/recipes', 'recipe.create'),
  route('GET', '/v1/inventory/recipes', 'recipe.list'),
  route('GET', '/v1/inventory/recipes/:id', 'recipe.get'),
  route('POST', '/v1/inventory/recipes/:id/actions', 'recipe.action'),
  route('POST', '/v1/inventory/recipes/:id/build', 'recipe.build'),
  route('POST', '/v1/inventory/de-kitting', 'dekit'),
  route('POST', '/v1/inventory/customization-work-orders', 'cwo.open'),
  route('GET', '/v1/inventory/customization-work-orders', 'cwo.list'),
  route('GET', '/v1/inventory/customization-work-orders/:id', 'cwo.get'),
  route('POST', '/v1/inventory/customization-work-orders/:id/actions', 'cwo.action'),
  route('POST', '/v1/inventory/kitting-work-orders', 'kwo.open'),
...
export function createScmHttpServer({ config, store, bus, verify, log = () => {} }) {
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      else if (name === 'recipe.list') {
        const p = url.searchParams
        result = await bus.queries.recipes(scope, { businessId: p.get('businessId'), productId: p.get('productId') || undefined, includeArchived: p.get('includeArchived') === 'true' })
      } else if (name === 'recipe.get') result = await bus.queries.recipe(scope, params.id, { quantity: url.searchParams.get('quantity') ?? undefined })
      else if (name === 'cwo.list' || name === 'kwo.list') {
        const p = url.searchParams
      ...
      else if (name === 'shelf.audit') {
        const p = url.searchParams
        result = await bus.queries.shelfLife(scope, { businessId: p.get('businessId'), thresholdDays: p.get('thresholdDays') || undefined, includeEmpty: p.get('includeEmpty') === 'true' })
      } else if (name === 'hygiene') result = await bus.queries.catalogHygiene(scope, { businessId: url.searchParams.get('businessId'), dormantDays: url.searchParams.get('dormantDays') ?? undefined })
      else if (name === 'replenishment') result = await bus.queries.replenishment(scope, { businessId: url.searchParams.get('businessId') })
      else if (name === 'intake.list') result = await bus.queries.catalogIntakes(scope, { businessId: url.searchParams.get('businessId'), limit: url.searchParams.get('limit') ?? undefined })
      else if (name === 'intake.by-code') result = await bus.queries.catalogIntakeByCode(scope, { businessId: url.searchParams.get('businessId'), code: url.searchParams.get('code') })
      else if (name === 'intake.get') result = await bus.queries.catalogIntake(scope, params.id)

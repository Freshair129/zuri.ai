import { z } from 'zod'
import { PURCHASE_ORDER_ACTIONS, SUPPLIER_ACTIONS } from '../lib/enums.js'
import { zInventoryCode } from '../inventory/inventory.js'
...
//   status machine (DRAFT → SENT → RECEIVED; SENT → SHORT_CLOSED as a short-close;
...
export const zPostReceipt = z.object({
  ...
  // @req FR-175, FR-165 — batch costs (freight, customs, handling) amortised
  // across counted lines into the movement's landed cost.
  batchCostSatang: z.number().int().nonnegative().optional(),
...
export function nextPurchaseOrderStatus(current, action) {
  switch (action) {
    ...
    case 'CLOSE':
      return current === 'SENT' ? 'SHORT_CLOSED' : null
    case 'CANCEL':

import { MARKET_RESOLUTION_STATUS } from './translate-raw-record.js'
...
// @tested services/market-intelligence/test/knowledge-identity-resolver.test.js
// Service copy (M1, 2026-09-24): moved verbatim from apps/server's Market module so the
// Market Intelligence service owns its core without importing apps/server. Until M3
// routes callers here, apps/server keeps the executing copy; contracts/v1 golden
// vectors are asserted by BOTH copies so they cannot drift silently.
// Renamed from gks-market-identity-resolver.js: the service sees only a
// KnowledgeIdentityReadPort (`reader.query`), never GKS itself.

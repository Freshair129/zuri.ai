...
import { ROLE_INVENTORY_MANAGER, ROLE_PROCUREMENT_BUYER } from '@/modules/identity/rbac'
...
let business, owner, buyer, receiver, member, noDomain, supplier, box, lotted, serial, service
...
describe('FR-161 GoodsReceipt', () => {
  beforeAll(async () => {
    ...
    receiver = makeViewer({ visibleBusinessIds: [b()], ownedBusinessIds: [], visibleDomains: DOMAINS, rolesByBusinessId: { [b()]: [ROLE_PROCUREMENT_BUYER, ROLE_INVENTORY_MANAGER] }, principal: { id: 'per-recv', code: 'PER-RECV', displayName: 'Receiver' } })
  ...
  it('AC-161.1 — a receipt posts counted lines into the Inventory ledger with the PO/GRN reference, and needs Inventory authority for that half', async () => {
    ...
    // A buyer without Inventory authority may still post the lines that touch no ledger.
    const freight = await postGoodsReceipt(order.id, { lines: [{ purchaseOrderLineId: lineOf(order, 'ค่าขนส่ง').id, qty: 1 }], supplierReference: 'DN-001' }, { viewer: buyer, now: NOW })
    expect(freight.receipt).toMatchObject({ code: 'GRN-20260906-001', supplierReference: 'DN-001', postedByPersonId: 'per-buyer' })
  ...
  it('AC-161.2 — the refusals: not SENT, unknown line, over-receipt with the per-line list, lot data on an uncounted line, the ladder', async () => {
    ...
    const boxLine = lineOf(order, 'Gift box')
    await expect(postGoodsReceipt(order.id, { lines: [{ purchaseOrderLineId: 'no-such-line', qty: 1 }] }, { viewer: owner })).rejects.toMatchObject({ status: 422, message: 'PROCUREMENT_RECEIPT_LINE_NOT_FOUND' })
    await postGoodsReceipt(order.id, { lines: [{ purchaseOrderLineId: boxLine.id, qty: 3 }] }, { viewer: owner, now: NOW })
    const error = await postGoodsReceipt(order.id, { lines: [{ purchaseOrderLineId: boxLine.id, qty: 2 }] }, { viewer: owner, now: NOW }).catch((e) => e)
    ...
    await expect(postGoodsReceipt(order.id, { lines: [{ purchaseOrderLineId: lineOf(order, 'Printing').id, qty: 1, lotCode: 'LOT-X' }] }, { viewer: owner })).rejects.toMatchObject({ status: 422, message: 'PROCUREMENT_RECEIPT_LINE_NOT_COUNTED' })
    await expect(postGoodsReceipt(order.id, { lines: [{ purchaseOrderLineId: boxLine.id, qty: 1 }, { purchaseOrderLineId: boxLine.id, qty: 1 }] }, { viewer: owner })).rejects.toThrow(/once per receipt/)
  ...
  it('AC-161.4 — a LOT-tracked line needs its lot and may carry the expiry; a SERIAL-tracked line names one serial per unit', async () => {
    ...
    await expect(postGoodsReceipt(order.id, { lines: [{ purchaseOrderLineId: teaLine.id, qty: 4 }] }, { viewer: owner })).rejects.toMatchObject({ status: 422, message: 'INVENTORY_LOT_REQUIRED' })
    expect(await prisma.goodsReceipt.count({ where: { purchaseOrderId: order.id } })).toBe(0)
    const first = await postGoodsReceipt(order.id, { lines: [{ purchaseOrderLineId: teaLine.id, qty: 4, lotCode: 'TEA-2026-09', expiresAt: '2027-09-01T00:00:00Z' }] }, { viewer: owner, now: NOW })
    ...
    await postGoodsReceipt(order.id, { lines: [{ purchaseOrderLineId: teaLine.id, qty: 6, lotCode: 'TEA-2026-09', expiresAt: '2030-01-01T00:00:00Z' }] }, { viewer: owner, now: NOW })
    ...
    await expect(postGoodsReceipt(order.id, { lines: [{ purchaseOrderLineId: gadgetLine.id, qty: 2, serialNos: ['SN-1'] }] }, { viewer: owner })).rejects.toMatchObject({ status: 422, message: 'INVENTORY_SERIAL_COUNT_MISMATCH' })
    const done = await postGoodsReceipt(order.id, { lines: [{ purchaseOrderLineId: gadgetLine.id, qty: 2, serialNos: ['SN-1', 'SN-2'] }] }, { viewer: owner, now: NOW })

...
describe('Marketing Content backup', () => {
  it('restores nonempty immutable content, review and decision rows in FK order', async () => {
    ...
    expect(restored).toMatchObject({ verdict: 'REJECT', brief: { businessId: business.id, version: 1 }, contentVersion: { payloadJson, payloadHash }, review: { reviewerId: 'reviewer', sequence: 2, rightsConfirmed: false, brandConfirmed: false } })
  })
})

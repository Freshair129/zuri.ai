...
// @req FR-046 — browser entry requires an explicit local session and one viewer-scoped response.
...
test.describe('FR-046 production-shaped entry boundary', () => {
  ...
  test('fails closed without a session and enters through the explicit demo session', async ({ page }) => {
    const unauthenticated = await page.request.get('/api/entry')
    ...
    const authenticated = await page.request.get('/api/entry')

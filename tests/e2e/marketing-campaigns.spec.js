...
async function createCampaignViaUi(page, title) {
  ...
  await page.getByLabel('Success metric intent', { exact: true }).fill(PLAN_FIELDS.successMetric)
  await page.getByLabel('Meta Ads', { exact: true }).check()
  await page.getByLabel('Budget', { exact: true }).fill('12000')
  await page.getByLabel('Currency', { exact: true }).fill('THB')
...
test.describe('Marketing Campaign first functional slice', () => {
  ...
  test('binds an approved PM receipt, reads real Plan and Timeline data, then persists closure', async ({ page, browser }, testInfo) => {
    ...
    try {
      ...
      await decisionForm.getByLabel('Decision', { exact: true }).selectOption('APPROVE')
      await decisionForm.getByLabel('Rationale', { exact: true }).fill('Approved for this campaign and immutable revision.')
      await decisionForm.getByLabel('Approval expires', { exact: true }).fill('2030-01-01T00:00')
      ...
      await page.reload()
      await page.getByLabel('Target Workspace', { exact: true }).selectOption(workspace.id)

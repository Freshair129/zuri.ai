...
describe('domain state projection', () => {
  it('committed output validates against the public state schema', () => {
    ...
    // `commerce` on 2026-09-07 (FR-158/FR-159, DOM-COMMERCE);
    // `procurement` on 2026-09-07 (FR-160/FR-161, DOM-PROCUREMENT).
    expect(Object.keys(state.domains).sort()).toEqual([
      'agent', 'asset-management', 'commerce', 'crm', 'identity', 'integration', 'inventory', 'knowledge', 'line-oa-studio', 'market-intelligence', 'platform-control', 'procurement', 'project-manager',
    ])
  })

...
describe('FR-164 supplier and purchase-order contracts', () => {
  ...
  it('the status machine: DRAFT → SENT → RECEIVED by receipt; SENT → CLOSED; open → CANCELLED; nothing from a closed one', () => {
    ...
    expect(nextPurchaseOrderStatus('SENT', 'CLOSE')).toBe('CLOSED')
    ...
    expect(nextPurchaseOrderStatus('CLOSED', 'UPDATE')).toBeNull()

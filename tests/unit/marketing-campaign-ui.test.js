...
const timelineTab = read('src/modules/marketing/components/campaigns/CampaignTimelineTab.jsx')
...
describe('FR-156 Campaign UI integration seams', () => {
  ...
  it('keeps execution reads on the server adapter and exposes PM progress caveats', () => {
    ...
    expect(planTab).toContain('formatProgressPercent')
    expect(planTab).toContain('current brief is newer; this receipt is historical')
    expect(timelineTab).toContain('The Project roadmap may include later handoff work')
  ...
  it('binds only explicit receipts, reuses Strategy evidence and keeps Team runtime out', () => {
    ...
    expect(decisionsTab).toContain('Payload hash:')
    expect(decisionsTab).toContain('Expires:')
    expect(decisionsTab).toContain('marketing-campaign-closure-reason')
    expect(decisionsTab).toContain('targetLabel')
    expect(decisionsTab).toContain('Receipt binding is disabled because the linked Strategy plan is archived')
    expect(read('src/modules/marketing/components/campaigns/CampaignBriefTab.jsx')).toContain('Campaign revisions are disabled')
